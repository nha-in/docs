...
Milestone 2 enables a [Health Information Provider](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#hip) (HIP) to create digital health records and to generate and link [care contexts](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#care-context) with a patient's [ABHA Address](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#abha-address). It also enables the HIP to facilitate record [discovery](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#discovery) through a Personal Health Record ([PHR](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#phr)) application, and to securely share encrypted health information across the ABDM ecosystem.
...
- **Data Transfer.** Health information is structured in accordance with the HL7 [FHIR](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#fhir) R4 standard. The FHIR bundle is encrypted and securely exchanged through the ABDM ecosystem.
...
M2 is applicable to healthcare facilities and systems that create and maintain digital health records, including hospitals, clinics, laboratories, pharmacies and diagnostic or imaging centres. A citizen pushing their own records from a PHR app builds [P2 Linking and records](/docs/pr-13/docs/hiecm/v3/milestones/p2), the patient side of M2.

## Prerequisites

Prior to implementation of Milestone 2, the following requirements shall be fulfilled:

1. The Healthcare Facility shall have a valid Health Facility Registry ([HFR](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#hfr)) ID, and the integrating software shall be registered and mapped to the facility as a Health Information Provider (HIP).
...
Each applicable health-information type shall be represented as a [FHIR](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#fhir) document Bundle. Depending on the applicable profile and use case, the Bundle may contain structured clinical data and, where permitted, an attachment such as a PDF document or image.
...
For the diagrams, "Your system" refers to the [HMIS](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#hmis), [LMIS](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#lmis), [EMR](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#emr) or other ABDM-compliant software used by the health facility to perform HIP functions. "HIE-CM" refers to the Health Information Exchange and Consent Manager that facilitates consent management and the secure exchange of health information within the ABDM ecosystem.
...
The HIP generates a [link token](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#link-token) using the patient's ABHA address and demographic details, which is then shared with the patient for authentication. Upon successful verification, a linking token valid for 6 months is created, and the patient's care context is linked to the corresponding ABHA address.

Step 8 is the acknowledgement. Step 9 is the answer. The link is confirmed only when the callback arrives at `/api/v3/link/on_carecontext` on your bridge, so do not mark a record as linked on the strength of the 202. See [the outcome of a care context linking call](/docs/pr-13/docs/hiecm/v3/api/m2/endpoints/m2-on-carecontext-result).
...
Steps 8 to 11 are the link init and confirm requests, answered by your `on-init` and `on-confirm` callbacks. Their fields are on the [M2 API reference](/docs/pr-13/docs/hiecm/v3/api/m2).
...
The HIU supplies the data push URL and its key material in the request. Encryption uses [ECDH](/docs/pr-13/docs/hiecm/v3/getting-started/glossary#ecdh), Elliptic Curve Diffie Hellman, over Curve25519. Call order: [M2 API reference](/docs/pr-13/docs/hiecm/v3/api/m2).
...
- The cases M2 is tested against: [M2 testing use cases](/docs/pr-13/docs/hiecm/v3/resources/testing/m2). Certification runs once, for the whole integration: [Go live](/docs/pr-13/docs/hiecm/v3/getting-started/going-live).
