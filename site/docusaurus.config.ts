...
// the two rules the tree cannot express live here: a version root's api/,
// reference/ and troubleshooting/ folders belong to the API references tab,
// not the overview, and a module's endpoints folder renders as its use-case
// groups from api-sidebar.json rather than a flat alphabetical list.

// The folders the API references tab owns. Kept in step with isApiRoute in
// src/config/navigation.ts and with the composition in sidebars.ts: a page
// that lights the API tab has to have an API sidebar to show.
const API_SIDE = ['api', 'reference', 'troubleshooting'];
...
async function sidebarItemsGenerator({defaultSidebarItemsGenerator, ...args}: any) {
  ...
  if (/^[^/]+\/[^/]+$/.test(dirName)) {
    // A version root is the overview sidebar; its api/, reference/ and
    // troubleshooting/ folders render in the API references tab instead.
    return items.filter((item: any) => {
      const id = firstDocId(item);
      return !(
        id &&
        API_SIDE.some((folder) => id.startsWith(`${dirName}/${folder}/`))
      );
    });
  }
