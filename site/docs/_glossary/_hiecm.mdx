...
Laboratory Management Information System, NHA's term for laboratory software. An LMIS sits on the provider facing side of the HIE-CM, the same side an [HMIS](#hmis) does: a [HIP](#hip) whenever it publishes a report, and an [HIU](#hiu) on the rarer occasions it reads a patient's history to interpret a result.
