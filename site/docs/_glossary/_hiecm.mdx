...
Information Management System, the umbrella term for the software a health facility runs: an [HMIS](#hmis) in a hospital, an [EMR](#emr) in a clinic, a [LIMS](#lims) in a laboratory, a [PMS](#pms) in a pharmacy. It names a category of software, not a role, and it does not matter which one you build: the facility is the [HIP](#hip) when it publishes and the [HIU](#hiu) when it fetches, so the work is [M2](#m2) and [M3](#m3) either way.
