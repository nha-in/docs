...
<span id="hmis"></span><span id="his"></span><span id="hims"></span>

### HMIS, HIS, HIMS

The software a hospital runs day to day: registration, visits, orders, results and billing. It goes by three names. HMIS is Hospital Management Information System, HIS is Hospital Information System, HIMS is Hospital Information Management System. One system, three names, one integration.

In ABDM it acts as a [HIP](#hip) when it publishes a record and an [HIU](#hiu) when it fetches one, so most need [M2](#m2) and [M3](#m3) rather than one or the other, and it must implement every [HI type](#hi-type). See [Hospital, lab and pharmacy systems](/docs/hiecm/v3/concepts/hip-hiu).

`HIS-` is also the prefix on every error code the HPR and the HFR return. Those are [M4](#m4) registry errors and have nothing to do with hospital software: see [the HPR and HFR call list](/docs/hiecm/v3/api/m4/undocumented).

### HRP

Health Repository Provider. HRP is the system that actually holds the records, written as "HRP/HIP" because the same system usually plays both parts. If you run an [HMIS](#hmis) or a [LIMS](#lims) and you are integrating M2, HRP means you.
...
Pharmacy Management System: the software a pharmacy runs to dispense and to keep its stock. It acts as an [HIU](#hiu) when it reads the prescription it is dispensing against, and as a [HIP](#hip) when it links what it dispensed. See [Hospital, lab and pharmacy systems](/docs/hiecm/v3/concepts/hip-hiu).
