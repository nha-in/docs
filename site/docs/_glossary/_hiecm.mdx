...
<span id="emr"></span><span id="ehr"></span>

### EMR, EHR

Electronic Medical Record and Electronic Health Record: the clinical system a hospital or a clinic records consultations, prescriptions and results in. The distinction drawn is that an EMR holds one provider's record of what happened in their own building, and an EHR follows the patient across providers. Vendors use the two words for the same product, so read which one a document means from what it describes rather than from the letters.

In ABDM it acts as a [HIP](#hip), linking care contexts in [M2](#m2). ABDM does not build an EHR as a database: the records stay with the HIP that created them, and the [HIE-CM](/docs/hiecm/v3/getting-started/glossary#hie-cm) plus consent is what lets another provider assemble the picture. The patient's own view of it is a [PHR](/docs/hiecm/v3/getting-started/glossary#phr) app. See [Hospital, lab and pharmacy systems](/docs/hiecm/v3/concepts/hip-hiu).
