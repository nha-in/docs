...
sidebar_position: 2
description: How a beneficiary's policy is found by ABHA, member ID or mobile, which code to address, and how a payer links a policy to ABHA.
...
Before any claim-side call, the beneficiary has to be matched to a policy, and the policy tells you which participant to address. This page covers the lookup and the link that makes it work. The [Participant registry](/docs/nhcx/v1/registries/participant) covers how to find the payer in the first place.
