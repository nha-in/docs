...
| --- | --- | --- | --- |
| [Provider: check coverage eligibility](/docs/nhcx/v1/api/eligibility/endpoints/eligibility-v1-coverageeligibility-check) | Provider | `POST /v1/coverageeligibility/check` | Provider asks the payer, via NHCX, whether a beneficiary's policy is in force, what it covers and which documents a preauth will need. |
| [Payer: send the coverage eligibility response](/docs/nhcx/v1/api/eligibility/endpoints/eligibility-v1-coverageeligibility-on-check) | Payer | `POST /v1/coverageeligibility/on_check` | Payer returns the CoverageEligibilityResponse (policy in force, benefits, auth requirements) or an error to the provider through NHCX. |
