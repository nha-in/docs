...
sidebar_position: 0
description: "The Pre-authorisation calls on NHCX: what each one does, the hosts they go to, and the guides that use them."
source: nhcx-package/apis/03-preauth
generated: true
---
...
| --- | --- | --- | --- |
| [Provider: submit a pre-authorisation](/docs/nhcx/v1/api/preauth/endpoints/preauth-v1-preauth-submit) | Provider | `POST /v1/preauth/submit` | Provider submits, resubmits, enhances or answers a query on a pre-authorisation Claim bundle (Claim.use preauthorization); NHCX routes it to the payer. Also sent as pre-authorisation enhancement. |
| [Payer: send the pre-authorisation response](/docs/nhcx/v1/api/preauth/endpoints/preauth-v1-preauth-on-submit) | Payer | `POST /v1/preauth/on_submit` | Payer returns the ClaimResponseBundle for a pre-authorisation (approved, partially approved, queried or rejected) to the provider via NHCX. |
...
- [Resubmission and cancellation of a pre-authorisation](/docs/nhcx/v1/roles/provider/preauthorisation#follow-ups-on-the-same-case): resubmission on workflow `121`, cancellation on `PC01`, answered on `PC02`
