...
import AdapterDownload from '@site/src/components/docs/AdapterDownload';
...
```bash
NHCX_ENV=sandbox
NHCX_PARTICIPANT_ID=1000003463@hcx
NHCX_CLIENT_ID=your-client-id
NHCX_CLIENT_SECRET=your-client-secret
NHCX_CALLBACK_URL=https://your-hmis.example.org/nhcx/callback
```
...
```bash
mkdir -p data
docker run --rm --env-file .env -v "$PWD/data:/data" ghcr.io/nha-in/nhcx-adapter:main cert generate
docker run -d --name nhcx-adapter --env-file .env -v "$PWD/data:/data" -p 8090:8090 ghcr.io/nha-in/nhcx-adapter:main
docker logs -f nhcx-adapter
```
