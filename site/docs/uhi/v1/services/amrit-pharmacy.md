...
| --- | --- | --- |
| [EUA](/docs/abdm/v3/glossary#eua) | You, or any [PHR](/docs/abdm/v3/glossary#phr) app | Sends the `search`. Receives the `on_search`. Renders the store list. |
...
| [HSPA](/docs/abdm/v3/glossary#hspa) | HLL Lifecare Limited | Queries the AMRIT store database. Builds the `on_search` catalog. |

You do not build a [HSPA](/docs/abdm/v3/glossary#hspa) unless you are HLL or their technology partner.
...
Each `providers[]` entry is one AMRIT store. Three things here do not appear in other UHI services: the pharmacy type on the fulfillment, the store opening and closing times, and the [HFR](/docs/abdm/v3/glossary#hfr) ID tag.
...
- The other pharmacy discovery service on UHI: [Jan Aushadhi Kendra discovery](/docs/uhi/v1/services/jan-aushadhi-kendra)
