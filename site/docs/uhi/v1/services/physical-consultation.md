...
Physical consultation is the fullest service on [UHI](/docs/abdm/v3/glossary#uhi): a patient searches for a doctor, sees real slots and fees from clinics they have no prior relationship with, books one, and receives a 4-digit PIN to present at the clinic. After this page you will know the four stages, the endpoints each role exposes, and the fields on every call, as [NHA](/docs/abdm/v3/glossary#nha) documents them.
...
| Actor | Role here |
| --- | --- |
| [EUA](/docs/abdm/v3/glossary#eua) | The patient facing app. Searches, books, shows the PIN and the status. |
| [HSPA](/docs/abdm/v3/glossary#hspa) | The provider platform. Holds doctor profiles and slots, confirms bookings, generates the PIN, drives the lifecycle. |
| HSP | The hospital, clinic or doctor. The HSPA is its digital interface. |
| [Gateway](/docs/abdm/v3/glossary#gateway) | NHA's routing layer. Involved in discovery only. |
...
| `fulfillment.agent.id` | string | No | Doctor [HPR](/docs/abdm/v3/glossary#hpr) address, for example `drmehra@hpr.ndhm` |
...
| `order.customer.id` | Yes | Patient [ABHA](/docs/abdm/v3/glossary#abha) address, for example `rahul.k001@sbx` |
...
- [M2](/docs/abdm/v3/api/m2), the prerequisite
