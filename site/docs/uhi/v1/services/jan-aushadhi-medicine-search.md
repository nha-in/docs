...
Change `fulfillment.type` to `JANAUSHADHI_KENDRA` and put the `medicineId` in `item.descriptor`. Add whichever location filter your user chose: state and district, pincode, or GPS and radius. The filter fields are the same as on [Jan Aushadhi Kendra discovery](/docs/uhi/v1/services/jan-aushadhi-kendra).
...
Every other field on the provider record is documented in the [Kendra discovery field reference](/docs/uhi/v1/services/jan-aushadhi-kendra#field-reference).
...
- Kendra discovery without a medicine filter: [Jan Aushadhi Kendra discovery](/docs/uhi/v1/services/jan-aushadhi-kendra)
