...
This is the database behind NHA's sandbox portal: the site where you register
your organisation, declare which milestones you will build, and apply for
sandbox exit. It is NHA's own store, not an [ABDM](/docs/overview/glossary#abdm)
API. Nothing on this page is an endpoint you can call. Read it when you want to
know what the portal records about your application, and what a status you see
on screen is called underneath.

:::note[Documented, not verified]
This page follows NHA's published document for the sandbox database data
dictionary v1.0. Nothing here has been run against the ABDM sandbox from this
repository, so treat request and response shapes as unconfirmed.
:::

## What the source contains

NHA's spreadsheet has five sheets. Three of them are transcribed on this page.
...
Each section below is one database table. The columns are:

- **Field**: the column name as NHA spells it.
- **Type**: the Postgres type from the source.
- **Nullable**: `No` means the column is `NOT NULL`.
- **Meaning**: NHA's description, kept as written.

Three things save you time later.

`sd_id` is the self declaration identifier. It is the join key across most
tables. `id_public` is a second, public facing identifier that many tables
carry alongside the primary key.

Several column names are misspelled in the schema itself: `messege`,
`previlege`, `redy_for_testing`, `suporting_doc`, `integratin_phase`,
`user_jorny`, `admin_coment`, `sare_date`. They are reproduced as they are,
because those are the real names.

The descriptions are NHA's. Where a description is thin, for example `flag` on
`sd_exit`, we have not padded it out with a guess.

### Abbreviations used in the descriptions

NHA's descriptions use these short forms.

| Short form | What it means |
| --- | --- |
| [ABDM](/docs/overview/glossary#abdm), NDHM | The programme. NDHM is the former name and both appear in the source. |
| [PHR](/docs/overview/glossary#phr) | Personal health record application. |
| [HIU](/docs/overview/glossary#hiu) | Health information user. |
| [UHI](/docs/overview/glossary#uhi) | Unified Health Interface. |
| [NHCX](/docs/overview/glossary#nhcx) | National Health Claims Exchange. |
| [HMIS](/docs/overview/glossary#hmis) | Hospital management information system. |
| [OTP](/docs/overview/glossary#otp) | One time password. |
...
| Reference data and content | [`std_data`](#std_data) | Telephone dialling codes, with the state and charging area names for each |
...
The main registration record for an organisation on the sandbox portal. `sd_id` is the identifier every other table joins on.
...
Password values and their timestamps, held apart from `sd_login`.
...
Web session state for a signed in portal user.
...
The registered address attached to a self declaration.
...
A short record marking an application as an active integrator.
...
What an organisation declared it would build, and the start and end dates it gave for each milestone.
...
The review trail for an application: administrator decision, four HTC review stages, production access status.
...
The sandbox exit application: contacts, integration details, uploaded evidence and every review stage on it.
...
Files uploaded with a sandbox exit application.
...
The list of document types an application may upload.
...
WASA issue and expiry dates against a client identifier and a bridge identifier.
...
Client identifiers issued for HIU registrations.
...
Requests to work on UHI, with the service type and the stated intent.
...
Organisation details captured for a health claims exchange registration.
...
The registered address for an `hcx` record.
...
The NHCX application and its administrative review status.
...
Roles inside the portal and their landing pages.
...
Portal screens and menu entries.
...
Before and after values for a changed record, with who changed it.
...
Request level audit: endpoint, method, user agent, correlation identifier and payload.
...
Token generation and email dispatch events against a client.
...
Notifications sent, by template, recipient and delivery status.
...
Support messages raised through the portal.
...
STD dialling codes with their state, LDCA and SDCA names.
...
Sessions listed on the portal, with joining links.
...
Exceptions raised by NHA's AWS Database Migration Service tasks. It is infrastructure, not portal data.
...
carries the same column list as `sd_exit`. The source does not say which of
that pair the portal writes to. All four repeat a column list you have already
read, so they are summarised here instead of listed in full.
...
The three `sd_login` copies are owned by `sandboxportaluser`. Every other
table in the schema is owned by `appprdusrsandbox`. The digits in
`sd_login_bk_16062026` and `sd_login_bk_20012026_updt` read as dates. The
source does not say what they mark.

## Indexes

Every index in the source, with the columns it covers. The full `CREATE INDEX`
statements are in NHA's spreadsheet if you need them.
...
Two sheets are left out of this page.

**Sequences** lists 34 Postgres sequences, one per auto incrementing key, with
start value, minimum, maximum and increment. Every one of them increments by 1
and does not cycle. There is nothing in it you act on.

**Def values** lists 104 column defaults. They are of two kinds:
`nextval('<sequence>'::regclass)` on identifier columns, and
`NULL::character varying` on text columns. Again, nothing to act on.

If you need either sheet, it is in NHA's sandbox document pack as the data
dictionary spreadsheet, v1.0.

## What this page does not tell you

This dictionary describes NHA's portal, not the [ABDM](/docs/overview/glossary#abdm)
APIs. It carries no request shape, no response shape and no endpoint. Registry
identifiers such as [ABHA](/docs/overview/glossary#abha) numbers do not appear
in it at all.

For the APIs, start at [choose your gateway](/docs/api). For what the sandbox
is and how to sign up for it, read
[get started](/docs/overview/get-started).
