...
Blood bank discovery is the smallest service on [UHI](/docs/overview/glossary#uhi) and the quickest to build. One call pair. Your app sends a search with a blood group, a component and a location, and receives back a list of blood banks with unit counts, addresses and phone numbers. This page follows [NHA](/docs/overview/glossary#nha)'s onboarding document, version 1.0.

:::note[Documented, not verified]
This page follows NHA's published document for the UHI blood bank discovery
service, version 1.0. Nothing here has been run against the ABDM sandbox from
...
Discovery only: `search` and `on_search`. There is no booking or reservation workflow in this service. Scope your interface to discovery, and put the blood bank's phone number somewhere the user cannot miss it, because calling is how a unit gets held.
...
| Role | What it does |
| --- | --- |
| [EUA](/docs/overview/glossary#eua) | A patient or clinician facing app. Sends searches, receives results at its callback URL |
| [HSPA](/docs/overview/glossary#hspa) | A blood bank management system or aggregator. Answers searches from its own inventory database |

At the time of NHA's document there was one registered blood bank HSPA on the network: e-RaktKosh, the centralised blood bank management system.
...
| 2 | [Gateway](/docs/overview/glossary#gateway) | Returns HTTP 200 with an acknowledgement. This confirms receipt only |
...
There is no end signal. Responses land one at a time and nothing tells you the last one has arrived. NHA recommends a timeout window of 10 to 15 seconds, and displaying results as they come in rather than waiting for the set.
...
Support both. GPS returns incomplete results where blood bank density is low, so state and district is the fallback your users need, and it is the only option when GPS is unavailable.
...
To search every blood group at once, set `item.descriptor.name` to `All` and `item.descriptor.code` to `-1`. Use that when the patient needs blood urgently and any available group is worth showing.
...
Availability is not a field on the item. Each provider declares a small set of `fulfillments`, one meaning `Available` and one meaning `NotAvailable`. Each blood group item then points at one of them through `fulfillment_id`. So a group with a unit count of 16 can still be unavailable if its `fulfillment_id` resolves to the `NotAvailable` record. Resolve the link before you show a number to a user.
...
- [ABDM](/docs/overview/glossary#abdm) [M2](/docs/api/hie-cm/m2) with [HIE-CM](/docs/overview/glossary#hie-cm) completed. NHA states this as a hard prerequisite for any UHI service.
