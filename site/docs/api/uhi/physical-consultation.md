...
Physical consultation is the fullest service on [UHI](/docs/overview/glossary#uhi). A patient searches for a doctor, sees real slots and fees from clinics they have no prior relationship with, books one, and receives a 4-digit PIN to present at the clinic. After the visit the clinic pushes state changes back to the app. This page follows [NHA](/docs/overview/glossary#nha)'s onboarding document, version 2.0.

:::note[Documented, not verified]
This page follows NHA's published document for the UHI physical consultation
...
## Phase, stated plainly

Two different phase numbers are in play, and confusing them wastes time.

- **NHA's service phase.** NHA describes the full consultation lifecycle as Phase 1 of this service, and it is live and open for onboarding. Online payment and refunds are Phase 2 of the service and are described as in ideation. Today the only payment model is pay on visit.
- **This portal's phase.** UHI is Phase 2 here. These pages are orientation. We have run nothing against the sandbox.
...
| Actor | Role here |
| --- | --- |
| [EUA](/docs/overview/glossary#eua) | The patient facing app. Searches, books, shows the PIN and the status. |
| [HSPA](/docs/overview/glossary#hspa) | The provider platform. Holds doctor profiles and slots, confirms bookings, generates the PIN, drives the lifecycle. |
| HSP | The hospital, clinic or doctor. The HSPA is its digital interface. |
| [Gateway](/docs/overview/glossary#gateway) | NHA's routing layer. Involved in discovery only. |
...
Your EUA posts to the gateway. The gateway validates your signature and the domain, then forwards to every registered HSPA in that domain.

```http
POST https://uhigatewaysandbox.abdm.gov.in/api/v1/uhi/search
```

Search filters go in `message.intent`.

| Field | Type | Required | What it is |
| --- | --- | --- | --- |
| `fulfillment.type` | string | Yes | `Physical`. Case sensitive |
| `fulfillment.agent.name` | string | No | Doctor name, for a name search |
| `fulfillment.agent.id` | string | No | Doctor [HPR](/docs/overview/glossary#hpr) address, for example `drmehra@hpr.ndhm` |
...
NHA asks HSPAs to send the full `agent.tags` set including `@abdm/gov.in/hip_id`. That ID is what lets the prescription or other records generated at the visit be pulled later.

### Second search, direct

After the patient picks a doctor, your EUA sends a second `search` straight to the HSPA's `provider_uri`, asking for that doctor's slots in a time window. This one carries `provider_id` and `provider_uri` in the context, and echoes the `provider`, `fulfillments` and `items` blocks from the first `on_search` so the HSPA knows which doctor is meant. The HSPA answers with a second `on_search` scoped to that doctor.
...
| `order.customer.id` | Yes | Patient [ABHA](/docs/overview/glossary#abha) address, for example `rahul.k001@sbx` |
...
The HSPA answers with the order ID, an itemised quote, and five terms objects the patient has to accept.
...
Send the whole order back with every term at `AGREED`. Any term still at `INITIATED` and the HSPA rejects the call.

Use the `order.id` the HSPA assigned in `on_init`, not any ID you generated.
...
The HSPA has two more obligations at this point. It sends an exact copy of the `on_confirm` payload to the gateway audit endpoint listed under [gateway endpoints](#gateway-endpoints), and it sets the communication tags on the fulfilment so the EUA knows what channels exist:
...
`status` carries only `order.id`. The HSPA answers with the full order object, and you replace your stored state with it. NHA describes this as a reconciliation mechanism, not a polling loop.
...
Your `/on_cancel` handler has to cope with both directions. It receives the HSPA's acknowledgement of a patient cancellation, and it also receives HSPA-initiated cancellations when a doctor cancels. Read the `cancelledby` tag to tell them apart.
...
The audit copies are an HSPA obligation for compliance traceability. They are not optional.
...
The `on_init` terms array carries the text the patient reads before confirming. NHA publishes sample clauses and states which content is mandatory in substance. Two points carry through all of them.
...
Read section 9 of NHA's document for the full sample text before you write your own.
...
- [M2](/docs/api/hie-cm/m2), the prerequisite
