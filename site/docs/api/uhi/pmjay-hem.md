...
PM-JAY is India's national health insurance scheme. HEM is the Hospital Empanelment Management system behind it, and it knows which hospitals are empanelled right now. This service puts that data in your app. Your user picks a state, and optionally a district, a speciality, a hospital name, a pincode or their GPS location. You get back hospitals that are currently empanelled, with their specialities, empanelment date, address and a nodal officer number. Discovery only. Booking and referral are a later phase.

:::note[Documented, not verified]
This page follows NHA's published document for PM-JAY HEM v1.1, March 2026. Nothing here has been run against the ABDM sandbox from this repository, so treat request and response shapes as unconfirmed.
:::

UHI is Phase 2 for this portal, so this page is orientation rather than a certified integration path. One thing to know about the source: the file NHA circulated is named v1.4, and its cover page reads Version 1.1, March 2026. This page follows the document contents.

## Who does what

This is an [EUA](/docs/overview/glossary#eua) side integration. There is one [HSPA](/docs/overview/glossary#hspa) for PM-JAY HEM and [NHA](/docs/overview/glossary#nha) operates it. No third party builds a HSPA for this service.

| Party | Who runs it | What it does here |
| --- | --- | --- |
| EUA | You, or any [PHR](/docs/overview/glossary#phr) app | Sends the `search`. Receives the `on_search`. Renders the hospital list. |
| [UHI](/docs/overview/glossary#uhi) Gateway | NHA | Validates and routes the `search`. Relays the `on_search` back to you. |
| PM-JAY HEM HSPA | NHA | Queries the HEM database. Builds the `on_search` catalog. |

Signing, onboarding and Gateway base URLs are shared across UHI. See [UHI services](/docs/api/uhi).

## Before you onboard

One prerequisite is hard. Your application must have completed [HIE-CM](/docs/overview/glossary#hie-cm) Milestone 2 as an [ABDM](/docs/overview/glossary#abdm) compliant application. NHA's document states that applications without M2 cannot be onboarded onto UHI services, PM-JAY HEM included. If you have not done that yet, start at [M2](/docs/api/hie-cm/m2).
...
Two calls. The HTTP 200 from the Gateway confirms receipt, nothing more.
...
NHA's document gives these samples as an abbreviated field listing with dotted paths, not as literal JSON request bodies. They are reproduced in that form below. Build your actual JSON from the field reference underneath, where the nesting is explicit.
...
NHA lists these for the current phase. Design around them rather than discovering them in production.
...
NHA's document carries 29 test cases across five categories, to be run against the UHI sandbox before you request production sign off. The shape of them:
...
We have not run any of these. Ask NHA for the full test case sheet and the UHI Postman collection when you onboard.

## Phase scope

PM-JAY HEM on UHI is a three phase plan and only Phase 1, discovery, is open for onboarding. Booking and referral workflows, provider dashboards and CSC kiosk search are named as Phase 2. [ABHA](/docs/overview/glossary#abha) linked discharge summaries and multilingual voice are named as Phase 3. NHA has not published dates for either.

## Next

- Shared UHI protocol, signing and onboarding: [UHI services](/docs/api/uhi)
- The M2 prerequisite: [HIE-CM M2](/docs/api/hie-cm/m2)
