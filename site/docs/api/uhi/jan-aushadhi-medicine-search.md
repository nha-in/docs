...
This is the two step version of Jan Aushadhi discovery. First your user searches for a medicine by name and gets back the generic medicines that match, with price and pack size. Then your user picks one, and you search again for the Kendras that carry it, with a stock flag on each. Both steps use the same `search` and `on_search` pair as [Jan Aushadhi Kendra discovery](/docs/api/uhi/jan-aushadhi-kendra). Only `fulfillment.type` and `item.descriptor` change.

:::note[Documented, not verified]
This page follows NHA's published documents for Jan Aushadhi Kendra medicine search v0.3 and Kendra search v0.3. Nothing here has been run against the ABDM sandbox from this repository, so treat request and response shapes as unconfirmed.
:::
...
The same three parties as every [UHI](/docs/overview/glossary#uhi) service. Your app is the [EUA](/docs/overview/glossary#eua). [NHA](/docs/overview/glossary#nha) runs the Gateway. PMBI runs the [HSPA](/docs/overview/glossary#hspa) that holds the medicine and stock data. Signing, onboarding and the Gateway base URLs are on [UHI services](/docs/api/uhi).
...
NHA's document writes its sample values as an annotation pair, for example `"id": "medicineId - 78299"`. The samples on this page show the value only. The `catalog.descriptor` fields carry descriptive prose in NHA's document rather than real values, so they appear here as named placeholders. What each field means is in the tables below.
...
Everything else matches a plain Kendra search. Change `fulfillment.type` to `JANAUSHADHI_KENDRA` and put the `medicineId` in `item.descriptor`. Add whichever location filter your user chose: state and district, pincode, or GPS and radius. The filter fields are the same as on [Jan Aushadhi Kendra discovery](/docs/api/uhi/jan-aushadhi-kendra).
...
The provider records are Kendra records, the same shape as Kendra discovery, with one addition. Each Kendra carries an `items[]` entry for the medicine you asked about, and that entry has a `flag` telling you whether it is in stock.
...
Every other field on the provider record is documented in the [Kendra discovery field reference](/docs/api/uhi/jan-aushadhi-kendra#field-reference).
...
One more thing worth knowing. NHA's stock `on_search` sample in the source document is not valid JSON. An `items` array is left unclosed part way through the first provider record. The sample above is transcribed from the second provider record in that document, which is complete.

## Next

- Kendra discovery without a medicine filter: [Jan Aushadhi Kendra discovery](/docs/api/uhi/jan-aushadhi-kendra)
