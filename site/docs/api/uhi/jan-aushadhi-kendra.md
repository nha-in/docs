...
Jan Aushadhi Kendras are the government generic medicine stores run under the Pradhan Mantri Bhartiya Janaushadhi Pariyojana. This service lets your app find them. Your app sends one `search`, and the store data comes back later in an `on_search` callback. There is no booking and no ordering in this service. Discovery only.

:::note[Documented, not verified]
This page follows NHA's published document for Jan Aushadhi Kendra Discovery Service v1.0, June 2026. Nothing here has been run against the ABDM sandbox from this repository, so treat request and response shapes as unconfirmed.
:::
...
Three parties are involved. The shared roles, signing and onboarding steps are on [UHI services](/docs/api/uhi). This page covers only what is specific to Jan Aushadhi Kendra.

| Party | Who runs it | What it does here |
| --- | --- | --- |
| [EUA](/docs/overview/glossary#eua) | You, or any [PHR](/docs/overview/glossary#phr) app | Sends the `search`. Receives the `on_search` on your callback URL. Renders the Kendra list. |
...
| [HSPA](/docs/overview/glossary#hspa) | PMBI, the Pharmaceuticals and Medical Devices Bureau of India | Queries the PMBI Kendra database. Builds the `on_search` catalog. |

PMBI operates the [HSPA](/docs/overview/glossary#hspa) for this service. You do not build a HSPA unless you are PMBI or their technology partner.

## Message flow

Two calls. The exchange is asynchronous. The HTTP 200 you get back from the Gateway is a receipt, not the result.
...
Match the `on_search` to the request that caused it by `context.transaction_id`. The [HSPA](/docs/overview/glossary#hspa) copies it from your `search`. Your `consumer_uri` must be a publicly reachable HTTPS endpoint.

## Service identity

These values are fixed for this service. They are what tells the Gateway to route your `search` to PMBI rather than to a teleconsultation or ambulance provider.
...
Signing and onboarding are shared across UHI. See [UHI services](/docs/api/uhi).

## Search filters

There is no mandatory location filter on this service. All five filter shapes below are independent, and they combine.
...
PMBI sends this to your `consumer_uri` through the Gateway. Each entry in `catalog.providers` is one enrolled Kendra. This sample is shortened to one provider record.
...
Two things in NHA's documents contradict each other. We have not run either against sandbox, so we cannot tell you which one the HSPA accepts.
...
- Searching for a medicine, then for the Kendras that stock it: [Jan Aushadhi medicine search](/docs/api/uhi/jan-aushadhi-medicine-search)
