...
happened since. The checks below are in the order we recommend, not a record of how
...
1. **Is the callback URL registered with the gateway?** Set it with
   ...
   call.
2. **Is that URL reachable from the public internet?** ABDM
...
4. **Is your endpoint returning a non success status?** A handler that
   ...
   delivery attempts stop is not documented yet. Respond quickly with a
