...
If your symptom is not here, or you already have an error code in hand,
the [error codes reference](/docs/hiecm/v3/reference/error-codes) is
organised the other way, by code.
...
`REQUEST-ID` is a fresh UUID you generate per request. It is the one value that
names the exact call you made, so log it and quote it.
