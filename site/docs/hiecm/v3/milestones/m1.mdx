...
<SkillInstall slug="abdm-m1" note="Every M1 call, its error codes and NHA's own test cases in one file: 55 operations, 89 codes, 122 cases." />
...
M1's certification cases are NHA's own, from the ABHA creation and verification
sheet. Search them, or filter to the mandatory ones.
