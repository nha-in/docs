...
<SkillInstall slug="abdm-m1" note="Every M1 call and its error codes in one file: 41 operations, 14 codes." />
...
The card is returned in the response rather than fetched from a separate link. The content type and encoding of the card and QR code responses are not yet published. See the [M1 API reference](/docs/hiecm/v3/api/m1) for the fields that are.
...
reference](/reference/hiecm-p1), written up as [creating an ABHA
...
address](/docs/hiecm/v3/api/m1/endpoints/m1-login-abha-address-mobile-otp/01-m1-post-v3-phr-web-login-abha-search) returns the
...
The [OTP request](/docs/hiecm/v3/api/m1/endpoints/m1-login-abha-address-mobile-otp/02-m1-post-v3-phr-web-login-abha-request-otp) carries a
...
Once it verifies, the [PHR profile](/docs/hiecm/v3/api/m1/endpoints/m1-login-abha-address-mobile-otp/04-m1-get-v3-phr-web-login-profile-abha-profile)
...
The two calls: [receive a patient's shared profile](/docs/hiecm/v3/api/m2/endpoints/m2-abdm-patient-share-hip/01-m2-post-v3-hip-patient-share) and [send the share acknowledgement](/docs/hiecm/v3/api/m2/endpoints/m2-abdm-patient-share-hip/02-m2-post-patient-share-v3-on-share). The patient's side is [P2 Linking and records](./p2#scan-and-share-at-a-facility).
