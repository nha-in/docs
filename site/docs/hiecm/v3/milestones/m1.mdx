...
- Two tokens exist and are not interchangeable: the gateway token in
  `Authorization`, the user token in `X-token`. The login
...
Face, fingerprint and iris each have their own request body on the enrolment call,
`POST /v3/enrollment/enrol/byAadhaar`. The reference has one sequence per method:
[face](/docs/hiecm/v3/api/m1/endpoints/m1-create-face/01-gateway-post-gateway-v3-sessions),
[fingerprint](/docs/hiecm/v3/api/m1/endpoints/m1-create-fingerprint/01-gateway-post-gateway-v3-sessions)
and [iris](/docs/hiecm/v3/api/m1/endpoints/m1-create-iris/01-gateway-post-gateway-v3-sessions).
...
The card is returned in the response rather than fetched from a separate link. Both the card and the QR code return a PNG image. See the [M1 API reference](/docs/hiecm/v3/api/m1) for the fields.
