...
Some people cannot use the OTP route, usually because the mobile registered against their Aadhaar is no longer theirs. Face authentication is the optional alternative. The person authenticates their face in the ABHA app on their own phone, which is how the transaction moves from your screen to theirs.
...
the full name as per Aadhaar, the date of birth, the gender, `stateCode` and
`districtCode`.
...
- **Create.** The same `enrol/byAadhaar` call, with `authMethods` set to `child`
  ...
  gender and `parentConsent`. The parent is authenticated first, and must be 18
  or older. The parent's user token goes in the `X-token` header.
...
Step 3 is a callback on the URL registered for your bridge, not a call you make. Answer it with a 200 at once and do the registration afterwards. Step 7 is your reply: `acknowledgement.status` is `SUCCESS` with `profile.tokenNumber` and the same `context`, or `FAILED` with an `error` code and message.
...
`profile.patient` carries the ABHA number, the ABHA address, name, gender, date of birth, mobile number and address. Match on the ABHA address. The counter context is yours to define: 1 to 250 characters, and never the facility id, the HIP id or the HIP name.
