...
and iris are a real route: the creation table above lists them as optional
for every integrator, and the enrolment call
...
- **KYC of a child ABHA** uses the profile OTP calls,
  `POST /v3/profile/account/request/otp` and `POST /v3/profile/account/verify`.
...
This route is not recommended for integrators. It is kept here,
...
enrolled from an identity document.
