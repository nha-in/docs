...
import matrix from '@site/src/data/test-matrix/m2.json';
...
<SkillInstall slug="abdm-m2" note="Every M2 call and callback, its error codes and NHA's own test cases in one file: 31 operations, 120 codes, 46 cases." />
...
lists the channels. The cases NHA runs are on this page, under [test
cases](#test-cases).
...
M2's certification cases are NHA's own, from the Building HIP sheet. Search
them, or filter to the mandatory ones.

Every case keeps the id NHA uses, so a case you pass here is a case you can
name in certification. Cases this portal added rather than took from the sheet
say so on the case itself.
