---
title: P3 Consent and notifications
sidebar_label: P3 Consent
sidebar_position: 11
sidebar_custom_props:
  roles: [phr]
description: 'The patient side of M3: subscriptions, auto approval, granting and revoking consent, and fetching the records a grant covers.'
verification: unverified
source: catalogue/openapi/hiecm/v3/hiecm-p3.yaml
sidebar_class_name: sidebar-icon sidebar-icon--file-check
---

import ApiLinks from '@site/src/components/docs/ApiLinks';

# P3 Consent and notifications

P3 is the other side of [M3 Retrieve](./m3). M3 is a requester asking for
records. P3 is the patient deciding, and being told each time.

A citizen fetching records is the
[HIU](/docs/hiecm/v3/getting-started/glossary#hiu), and the
[PHR](/docs/hiecm/v3/getting-started/glossary#phr) application implements that
side.

<ApiLinks module="p3" />

## In short

- Build for revocation from the start. A consent that worked yesterday can be
  withdrawn today, and that is the system working correctly.
- A subscription is how your app hears about changes to a user's ABHA address.
- An auto approval policy stops the user approving a request every time a
  hospital adds a record.
- A policy can be disabled and enabled again.

## Subscriptions and notifications

An approved subscription notifies your app by category, `LINK` or `DATA`.

You need screens to list subscriptions, approve them, deny them and edit them.
Editing covers health information types, purpose, categories and the time period.

<LegacyAnchor id="auto-approval" />

## Auto approval: subscribe once, approve every time

1. Set up an auto approval policy for an HIU with the
   [HIE-CM](/docs/hiecm/v3/getting-started/glossary#hie-cm).

While the policy is active, consent requests from that HIU are approved
automatically. Disable the policy and each request waits for the user instead.

## Consent management

| Capability | What it covers |
| --- | --- |
| View requests | Requesting HIU, purpose, data types, date range, validity, status |
| Modify a request | Access duration, record date range, data categories, validity period |
| Grant or deny | The decision goes back to the HIE-CM |
| View active consents | Who currently has access, and to what |
| Revoke | Withdraw a granted consent, ending the access it gave |

## Fetching and displaying records

Once a care context is linked to the user's ABHA address:

1. Your app receives the notification.
2. It creates a consent request for that record and sends it to the HIE-CM.
3. The consent is granted, automatically if a policy exists, otherwise by the
   user.
4. It raises a health information request with the approved [consent
   artefact](/docs/hiecm/v3/getting-started/glossary#consent-artefact).
5. The [HIP](/docs/hiecm/v3/getting-started/glossary#hip) sends the records
   across the network.
6. Your app stores and displays them.

## Next

- The calls and base URLs: [P3 API reference](/reference/hiecm-p3).
- Back to the four provider milestones: [Milestones](/docs/hiecm/v3/milestones).
- Take your integration to production: [Go live](/docs/hiecm/v3/getting-started/going-live).
