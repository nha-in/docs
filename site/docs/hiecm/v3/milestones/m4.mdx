...
<SkillInstall slug="abdm-m4" note="Every M4 call, its error codes and NHA's own test cases in one file: 13 operations, 150 codes, 183 cases." />
...
and what each one asks of you. The cases NHA runs are on this page, under [test
cases](#test-cases).
...
M4's certification cases are NHA's own, from the HFR and HPR sheets. Search
