...
- **Professionals registering.** An HPID is a verified identity in ABDM. The
  `hprType` categories include doctor, nurse, pharmacist and facility manager.
...
sequenceDiagram
    ...
    H-->>S: Demographic and address details, with the mobile number
