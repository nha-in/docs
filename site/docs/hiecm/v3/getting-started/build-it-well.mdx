...
| Request an OTP | No | Each request sends the person a new OTP |
| Enrol, or create an ABHA | No | A success you did not see still created an account |
| Your callback handler | It must be | Deduplicate on the id the callback carries before you apply any effect |
...
| Rate limited or locked out | Name the actual wait | Say something went wrong |
...
The first two are journey specific and each is explained where it happens, on
...
profile returned twenty eight. The rest is then either retyped by a person who already
...
[profile with `kycVerified` true](/docs/hiecm/v3/milestones/m1#abha-with-aadhaar) was proved
against Aadhaar, so its demographics are better evidence than anything typed at a
desk and your form should lock them. A
[profile with `kycVerified` false](/docs/hiecm/v3/milestones/m1#abha-address-with-a-mobile-number)
was not proved against Aadhaar, so it is correctable.

Two traps. Several of these fields can come back null, so a form that renders a blank labelled row for
