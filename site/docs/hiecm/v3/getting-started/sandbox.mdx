...
Register one base URL. We post to paths under it, and each callback that answers a call carries the
`REQUEST-ID` you sent on it in `response.requestId`, so you can match the answer to the question. Each path is named on
...
[The ABDM gateway](/docs/hiecm/v3/concepts/gateway#which-host) gives the gateway host for
sandbox and for production side by side.
