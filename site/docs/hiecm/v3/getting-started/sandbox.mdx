...
Create an account at <a href={sandboxLinks.home} target="_blank" rel="noopener noreferrer">the ABDM sandbox</a> and register
...
[NHA](/docs/hiecm/v3/getting-started/glossary#nha) reviews the request before it issues them.
NHA's FAQ says the application itself takes about 10 minutes, and that applications are
processed in a maximum of 5 working days. Nothing in ABDM unblocks until the credentials
arrive, so raise the request on day one.
...
- [Your first fifteen minutes](/docs/hiecm/v3/getting-started/first-fifteen-minutes) gives you
  a transcribed exchange and your own first call assembled, so you have something ready to run
