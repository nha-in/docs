...
Building with a coding agent? Get the M2 skill on [Build with AI](/docs/hiecm/v3/getting-started/build-with-ai).

## In short

- You act as the [HIP](/docs/hiecm/v3/getting-started/glossary#hip). A valid Facility ID and HIP registration come first.
- M2 is keyed to an ABHA address, so [M1](/docs/hiecm/v3/api/m1) has to work before M2 can.
- Hold a [link token](/docs/hiecm/v3/getting-started/glossary#link-token) per patient. NHA gives its validity as six months.
- Records go out as [FHIR](/docs/hiecm/v3/getting-started/glossary#fhir) R4 against the ABDM profiles.
- Four callbacks name a path and carry no payload in NHA's sources. Do not assume a body.
...
NHA certifies M2 individually, alongside the other milestones your role needs; see
