...
A health record your system creates is invisible to [ABDM](/docs/abdm/v3/glossary#abdm) until you link it, which means telling the [HIE-CM](/docs/abdm/v3/glossary#hie-cm) that a named group of records exists at your facility for a named person. Until that happens no patient can discover the record and no consent request can reach it; the calls that do it are in the [M2 guide](/docs/abdm/v3/api/m2).

## The unit that gets linked is a care context

You do not link a record. You link a [care context](/docs/abdm/v3/glossary#care-context), a group of a patient's records that your system defines. It carries two fields and nothing else.
...
[NHA](/docs/abdm/v3/glossary#nha)'s example of a good display name is "OPD records (X-Ray, Prescription) from 3rd March 2023": what kind of visit and when, not what was found. NHA recommends one care context per outpatient visit and one per inpatient admission.

## Why the record has to be linked first

The HIE-CM is data blind. It holds a map: which [ABHA address](/docs/abdm/v3/glossary#abha-address) has care contexts at which facilities. Linking puts an entry on that map, and everything downstream reads it.

- A [PHR](/docs/abdm/v3/glossary#phr) app shows a patient their records by reading their linked care contexts.
...
| --- | --- | --- |
| [HIP](/docs/abdm/v3/glossary#hip) initiated linking | The patient shared their ABHA address with you | You |
| Notification to mobile | You hold a mobile number, name, age and gender, but no ABHA address | You, and then the patient |
| [Discovery](/docs/abdm/v3/glossary#discovery) and link | The patient goes looking for old records from their PHR app | The patient |
...
**Discovery and link.** The request comes to you. The patient picks the facility they visited in their PHR app, and the HIE-CM forwards a discovery request to the [HRP](/docs/abdm/v3/glossary#hrp) or HIP behind it. You match against your own patients and reply with care contexts, and the patient picks which to link. Implementing discovery is mandatory for every HIP, even if every patient gives you an ABHA address at the counter, because a patient who visited two years ago did not.
...
Linking is authorised by a [link token](/docs/abdm/v3/glossary#link-token), not by your session token alone. The token ties your facility to one patient's ABHA address.
...
The full list is on [M2 errors](/docs/abdm/v3/api/m2/errors).

## Where this is implemented

- [Hospital, lab and pharmacy systems](/docs/abdm/v3/concepts/hip-hiu), the role that does the linking.
- [M2, linking and sharing](/docs/abdm/v3/api/m2), the call order for all three routes.
- [Consent](/docs/abdm/v3/concepts/consent), what happens once somebody asks for a linked care context.
- [How a record travels](/docs/abdm/v3/concepts/data-flow), what you do when that request arrives.
- [PHR applications](/docs/abdm/v3/phr), the patient side of discovery and linking.
