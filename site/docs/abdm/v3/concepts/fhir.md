...
Every health record that moves inside [ABDM](/docs/abdm/v3/glossary#abdm) travels as a [FHIR](/docs/abdm/v3/glossary#fhir) bundle, not a PDF in an envelope of your own design and not your database rows. This page gives the version, the profiles, the bundle shape and how to validate one; the packaging detail is in the [M2 guide](/docs/abdm/v3/api/m2).

## R4, with Indian profiles

ABDM uses FHIR release 4. Base FHIR is loose: it says a `MedicationRequest` exists, not which fields an Indian prescription must fill. That second layer is a profile, and for ABDM the profiles are published by the National Resource Centre for EHR Standards, NRCES, at [nrces.in/ndhm/fhir/r4](https://nrces.in/ndhm/fhir/r4/index.html). When [NHA](/docs/abdm/v3/glossary#nha)'s documents say "ABDM compliant FHIR", they mean R4 conforming to those profiles. Read the NRCES implementation guide for field level questions. It is not restated here.
...
NHA's M2 document lists eight. Implementing all of them is mandatory for an [HMIS](/docs/abdm/v3/glossary#hmis).

| Record type | What it holds | [HI type](/docs/abdm/v3/glossary#hi-type) code |
| --- | --- | --- |
...
| Wellness Record | Vitals, physical examination and general health data, often captured in a [PHR](/docs/abdm/v3/glossary#phr) app | `WellnessRecord` |
...
NHA's M2 document names the resources the reference [HIU](/docs/abdm/v3/glossary#hiu) service supports inside a bundle.
...
That is the skeleton. The full sample, with the Organization, the Encounter and the section entries, is on [M2 use cases](/docs/abdm/v3/api/m2/sequence).
...
| `Composition.attester.party` | References an `Organization` whose `identifier.value` is your [HIP](/docs/abdm/v3/glossary#hip) id as registered in the facility registry. Mode is `official`. |
...
- [M2 use cases](/docs/abdm/v3/api/m2/sequence), the full bundle sample and validation in context.
- [How a record travels](/docs/abdm/v3/concepts/data-flow), what happens to the bundle after you build it.
- [Care contexts and linking](/docs/abdm/v3/concepts/linking), how records are grouped and made findable.
- [Consent](/docs/abdm/v3/concepts/consent), where the HI type codes above are chosen and read.
