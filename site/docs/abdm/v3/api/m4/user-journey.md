...
Milestone 4 of [ABDM](/docs/abdm/v3/glossary#abdm) has three journeys: a professional gets an [HPID](/docs/abdm/v3/glossary#hpid) and an [HPR](/docs/abdm/v3/glossary#hpr) profile, a facility manager onboards a facility to the [HFR](/docs/abdm/v3/glossary#hfr), and a facility links its bridges so its software can act as a [HIP](/docs/abdm/v3/glossary#hip) or an [HIU](/docs/abdm/v3/glossary#hiu). This page shows the order of calls in each; field lists are on the [what NHA documents without a path](/docs/abdm/v3/api/m4/undocumented) page.
...
The professional authenticates against Aadhaar on a page NHA hosts. Your system never handles the Aadhaar number or [OTP](/docs/abdm/v3/glossary#otp): it handles the transaction ID and redirects to a URL NHA returns, valid for five minutes. After that, call generate Aadhaar link again.
...
The HIP name is what a patient sees in their [ABHA](/docs/abdm/v3/glossary#abha) or [PHR](/docs/abdm/v3/glossary#phr) app when they search for this hospital. NHA sets three rules: 15 characters or fewer, no special characters, unique for every bridge on a facility. Its example builds the name from the hospital name plus the bridge name.

A facility with a facility ID and a linked HIP bridge can do the [M2](/docs/abdm/v3/api/m2) work, linking care contexts and sharing records. With a linked HIU bridge it can do the [M3](/docs/abdm/v3/api/m3) work, requesting consent and fetching records. M4 is the registration step in front of either flow outside sandbox.

Next: [what NHA documents without a path](/docs/abdm/v3/api/m4/undocumented).
