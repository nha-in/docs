...
This page lists every call NHA's Milestone 4 document names, for the [HPR](/docs/abdm/v3/glossary#hpr) and the [HFR](/docs/abdm/v3/glossary#hfr). NHA pasted most request and response samples in as screenshots, which did not convert to text, so many calls have a name, a purpose and a parameter list but no method or path. Every gap is marked, and nothing here is invented.
...
The first call you make, and one of the few whose method, URL and body all survived in text. Same session pattern as [M1](/docs/abdm/v3/api/m1), issued by the [HIE-CM](/docs/abdm/v3/glossary#hie-cm) gateway.
...
NHA gives both hosts and labels neither. See [the note on environments](/docs/abdm/v3/api/m4#environments).
...
Three fields below are sent encrypted: the mobile number in mobile match, the [OTP](/docs/abdm/v3/glossary#otp) in the HPR mobile login, and the email and password in create HPID. All use the same public certificate, so fetch it once.
...
| 4 | Check HPID exists by Aadhaar | Returns the [HPID](/docs/abdm/v3/glossary#hpid) already registered for this Aadhaar, if there is one | Behaviour only |
...
| `hipName` | Yes | The name a patient sees in their [ABHA](/docs/abdm/v3/glossary#abha) or [PHR](/docs/abdm/v3/glossary#phr) app. 15 characters or fewer, no special characters, unique for every bridge on a facility |
| `type` | Yes | `HIP` or `HIU` |
...
- The order of calls, as diagrams: [M4 user journeys](/docs/abdm/v3/api/m4/user-journey).
