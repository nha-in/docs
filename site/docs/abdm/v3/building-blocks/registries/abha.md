...
[ABHA](/docs/abdm/v3/glossary#abha) is the Ayushman Bharat Health Account, the patient half of [Registries](/docs/abdm/v3/building-blocks/registries). It answers "who is this patient", and every record flow in [ABDM](/docs/abdm/v3/glossary#abdm) starts from that answer.
...
| How it is issued | After an Aadhaar based [KYC](/docs/abdm/v3/glossary#kyc) check passes | Chosen by the person, or issued as a default |
...
Store both. You match a patient record against the number, and you send the address when you link a [care context](/docs/abdm/v3/glossary#care-context) or ask for consent. NHA's M1 document issues the number only after a strong KYC process completes.
...
| --- | --- | --- | --- |
| Aadhaar [OTP](/docs/abdm/v3/glossary#otp) | A code sent to the Aadhaar linked mobile number | Mandatory | Mandatory |
...
- **Every number gets a default address**, the number with a suffix: `14digit@sbx` in [sandbox](/docs/abdm/v3/glossary#sandbox), `14digit@abdm` in production. The M1 Postman collection shows a `preferredAbhaAddress` field holding the 14 digits with the `@abdm` suffix and no hyphens.
...
- **An address can exist without a number.** NHA's PHR document says one can be created on the [HIE-CM](/docs/abdm/v3/glossary#hie-cm) from mobile number, name, age and gender, self declared and with no KYC. Expect accounts with no number behind them.
...
[M1](/docs/abdm/v3/api/m1) is the only milestone that writes to this registry. It covers creation, login, profile management and sessions. Login by mobile number, Aadhaar number, ABHA number and ABHA address are all four mandatory for both private and government integrators.

## What every other milestone assumes

- **[M2](/docs/abdm/v3/api/m2)** links a care context to the ABHA address and answers [discovery](/docs/abdm/v3/glossary#discovery) against it. See [Linking records](/docs/abdm/v3/concepts/linking).
- **[M3](/docs/abdm/v3/api/m3)** raises a consent request against the ABHA address. See [Consent](/docs/abdm/v3/concepts/consent).
- **[PHR applications](/docs/abdm/v3/phr)** sign a person in by ABHA address and show the records linked to it.

No flow starts without an ABHA, so M1 comes first even when your real goal is M2 or M3.

## Next

- [NHPR](/docs/abdm/v3/building-blocks/registries/nhpr), the professional and facility registries.
- [M1, ABHA identity](/docs/abdm/v3/api/m1), the guide.
- [M1 API reference](/reference/hiecm-m1).
- [Sandbox data dictionary](/docs/abdm/v3/reference/data-dictionary), test identities.
