...
[HFR](/docs/abdm/v3/glossary#hfr) is the Health Facility Registry, the places half of [NHPR](/docs/abdm/v3/building-blocks/registries/nhpr) alongside the [HPR](/docs/abdm/v3/building-blocks/registries/nhpr/hpr). It is a national directory of hospitals, clinics, diagnostic laboratories, imaging centres and pharmacies, and a facility has to enrol here before it can do anything else on [ABDM](/docs/abdm/v3/glossary#abdm).

## Why it blocks everything else

NHA's M2 document states the prerequisite plainly: a valid facility ID and registration in the [HIP](/docs/abdm/v3/glossary#hip) role, before the entity can create health records and share them. A product that has finished [M2](/docs/abdm/v3/api/m2) or [M3](/docs/abdm/v3/api/m3) against the sandbox cannot go live without this registry.
...
Which parts of the detailed layer are mandatory depends on the facility type, the type of service and the system of medicine, and NHA's rules are on [what NHA documents without a path](/docs/abdm/v3/api/m4/undocumented). Two shape your form: an inpatient or day care facility must submit at least one bed count greater than zero, and an imaging centre, diagnostic laboratory, blood bank or pharmacy need not submit medical infrastructure at all.
...
Both come from a person, not from your application. That is why [HPR](/docs/abdm/v3/building-blocks/registries/nhpr/hpr) comes first in a rollout, and why somebody in your organisation needs an [HPID](/docs/abdm/v3/glossary#hpid) with facility manager rights, role 2 or role 3, before you write a line of HFR code.
...
| Send and validate OTP to contact | Sends an [OTP](/docs/abdm/v3/glossary#otp) to the mobile number registered against a facility, then validates it. This proves control of a facility record you did not create |

Base URLs for every call on this page are on [NHPR](/docs/abdm/v3/building-blocks/registries/nhpr).
...
- [HPR](/docs/abdm/v3/building-blocks/registries/nhpr/hpr), which issues the token these calls need.
- [NHPR](/docs/abdm/v3/building-blocks/registries/nhpr), the parent page.
- [what NHA documents without a path](/docs/abdm/v3/api/m4/undocumented), parameter tables and `HIS-` error codes.
- [M2, linking and sharing](/docs/abdm/v3/api/m2), which needs this facility ID.
