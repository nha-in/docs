...
The gateway is [NHA](/docs/abdm/v3/glossary#nha)'s routing layer for [ABDM](/docs/abdm/v3/glossary#abdm): you never call a hospital, a lab or a [PHR](/docs/abdm/v3/glossary#phr) app directly, and the answer to a call arrives later at an endpoint you expose. It also issues the access token every other call carries.

## Gateway and HIE-CM are not the same thing

[HIE-CM](/docs/abdm/v3/glossary#hie-cm) is the service: patient identity, care context links and consent. [Read what it does](/docs/abdm/v3/building-blocks/hie-cm).
...
- **You get an acknowledgement, not an answer.** In NHA's [M3](/docs/abdm/v3/glossary#m3) consent flow the [HIU](/docs/abdm/v3/glossary#hiu) asks, the HIE-CM acknowledges with a consent request id, and the patient's decision comes back later. See [callbacks](/docs/abdm/v3/reference/callbacks).
- **You have to be reachable.** Half of [M2](/docs/abdm/v3/glossary#m2) is endpoints the gateway calls on your system. A [HIP](/docs/abdm/v3/glossary#hip) it cannot reach fails on someone else's logs, as `ABDM-1028 HIP is unavailable`.
...
|---|---|---|
| [M1](/docs/abdm/v3/api/m1) | Session tokens, and the calls that create and authenticate an ABHA identity | [M1 API reference](/reference/hiecm-m1) |
| [M2](/docs/abdm/v3/api/m2) | [Discovery](/docs/abdm/v3/glossary#discovery), care context linking, health information requests to a HIP | [M2 API reference](/reference/hiecm-m2) |
| [M3](/docs/abdm/v3/api/m3) | Consent requests, consent notifications, artefact fetches, data flow requests | [M3 API reference](/reference/hiecm-m3) |
| [M4](/docs/abdm/v3/api/m4) | Session tokens for the [HPR](/docs/abdm/v3/glossary#hpr) and [HFR](/docs/abdm/v3/glossary#hfr) registry calls | [M4 API reference](/reference/hiecm-m4) |

The gateway holds no health record. It routes the permission and the metadata.

## The session endpoint

One endpoint issues the token every other call carries. It is a real request in NHA's [M1](/docs/abdm/v3/glossary#m1) Postman collection, and NHA's [M4](/docs/abdm/v3/glossary#m4) document repeats it.
...
Send the token back as `Authorization: Bearer <ACCESS_TOKEN_FROM_SESSIONS_CALL>` on every other call. Headers per call, and the second token M1 login issues, are on [authentication](/docs/abdm/v3/reference/authentication). Interactive: [gateway API reference](/reference/hiecm-gateway).
...
- **Request signing.** Nothing describes a signature over the gateway request itself. Payload encryption and signing are described for health records, on the [M2](/docs/abdm/v3/api/m2) side.

## Next

- [Authentication](/docs/abdm/v3/reference/authentication), credentials and headers.
- [Callbacks](/docs/abdm/v3/reference/callbacks), the inbound half.
- [HIE-CM gateway](/docs/abdm/v3/building-blocks/hie-cm), the four modules.
- [Registries](/docs/abdm/v3/building-blocks/registries), who and what ABDM identifies.
- [Error codes](/docs/abdm/v3/reference/error-codes), what a rejection means.
