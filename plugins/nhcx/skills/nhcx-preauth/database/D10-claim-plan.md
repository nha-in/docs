...
- Otherwise the request needs the claim's policy code and the facility's HFR ID and participant code ([D1](D1-organization.md)) and sends A3 with `x-hcx-workflow_id` = `claim_no` [REF](../references/PAYERS.md#markers). Written: `status = 'fetching'`, `txn_id`, `correlation_id`, `requested_at` = now, `policy_code` (the claim's, required), `provider_id` (HFR ID). Nulled: `fetched_at`, `error_message`, `plan_identifier`, `plan_title`, `plan_type`, `sum_insured`, `response_json`. A send that fails writes `status = 'error'` and `error_message`.
