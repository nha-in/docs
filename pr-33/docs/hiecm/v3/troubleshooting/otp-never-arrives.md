...
You called the request OTP operation, got back a `txnId`, and the phone you are watching has not received an [OTP](/docs/pr-33/docs/hiecm/v3/getting-started/glossary#otp).

Before working through the checks, confirm the request call itself succeeded and returned a `txnId`. If it failed, work through [Everything returns 401](/docs/pr-33/docs/hiecm/v3/troubleshooting/everything-returns-401) or the code the response names on the [error codes reference](/docs/pr-33/docs/hiecm/v3/reference/error-codes) instead.

## Work through these in order

1. **Is this the Aadhaar mobile, and are you holding it?** In the Aadhaar OTP enrolment flow, only the mobile number registered against that Aadhaar number receives the OTP. That is not necessarily the phone the person is holding while they enrol. See the [M1 user journey](/docs/pr-33/docs/hiecm/v3/milestones/m1) for the full enrolment sequence, and confirm which number Aadhaar has on file before assuming delivery failed.
...
This symptom can surface as a rate limit code or the catch-all failure code, both on the [error codes reference](/docs/pr-33/docs/hiecm/v3/reference/error-codes).

[Next Still stuck? Walk the M1 user journey See exactly where the OTP request sits among the calls that create an ABHA.](/docs/pr-33/docs/hiecm/v3/milestones/m1)
