...
Consent is the permission layer of [ABDM](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#abdm): a record moves because a patient said yes to a named system, for a named reason, over a date range, for a fixed length of time. The calls that create and read it are in the [M3 guide](/docs/pr-32/docs/hiecm/v3/api/m3).

## Two objects, not one

| Object                                                                                  | What it is                                                                                                                                                                | Who creates it                                                                                     | Identifier          |
| --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------- |
| Consent request                                                                         | The ask. It names the patient by [ABHA](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#abha) address, the reason, the record types wanted, and the date range wanted. | The [HIU](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#hiu), through the gateway             | Consent request id  |
| [Consent artefact](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#consent-artefact) | The permission itself, created only if the patient grants the request. It is what a record holder checks before sending anything.                                         | The [HIE-CM](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#hie-cm), on the patient's decision | Consent artefact id |
...
- **The patient holds the decision**, in their [PHR](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#phr) app.
- **The HIE-CM holds the artefact.** As [consent manager](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#consent-manager) it asks the patient, records the answer, and tells requester and record holder.
- **The HIU holds an id, not a right.** It can stop working at any time.
- **The [HIP](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#hip) holds the check.** Before sending a record it validates that the artefact is active and that the dates asked for sit inside the dates it allows.
...
Why you want the records. See [purpose of use](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#purpose-of-use). These codes are a subset of the HL7 v3 PurposeOfUse value set at [terminology.hl7.org](http://terminology.hl7.org/ValueSet/v3-PurposeOfUse).
...
What kind of record you are asking for. See [HI type](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#hi-type). M3 supports these types as of writing:
...
The M2 error message for an invalid HI type lists these seven and adds `Invoice`. The two disagree by one value, so check the swagger before you send `Invoice`. What each type carries as a [FHIR](/docs/pr-32/docs/hiecm/v3/getting-started/glossary#fhir) bundle is on [FHIR and health record formats](/docs/pr-32/docs/hiecm/v3/concepts/fhir).
...
An auto approval policy works like this: the patient authorises the app once, the app registers the policy with the HIE-CM, and later requests under that policy are granted immediately. The patient can disable it, after which each record needs its own request again. This changes who taps the button, not the model. An artefact is still created, still carries an expiry, and can still be revoked. Detail is on [PHR applications](/docs/pr-32/docs/hiecm/v3/concepts/participants/phr).

## Where this is implemented

- [Hospital, lab and pharmacy systems](/docs/pr-32/docs/hiecm/v3/concepts/hip-hiu), the facility taking each role.
- [The ABDM gateway](/docs/pr-32/docs/hiecm/v3/concepts/gateway), which holds every artefact here.
- [M3 Health Information User, Health Information User Services](/docs/pr-32/docs/hiecm/v3/api/m3), the requesting side.
- [M2 Health Information Provider, Health Information Provider Services](/docs/pr-32/docs/hiecm/v3/api/m2), what a record holder validates.
- [How a record travels](/docs/pr-32/docs/hiecm/v3/concepts/data-flow), what happens next.
