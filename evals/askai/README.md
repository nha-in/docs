...
Run:
    ...
    npm run eval:askai:calibrate  # check the judge's grades on the latest run against the owner's
...
exists, a case that fails with a failure string not already recorded for it
in `runs/baseline.json` fails the build: the baseline records which failures
were already known, per case, not just which cases were failing, so a case
...
`eval:askai:calibrate` checks that and fails clearly while that file is
...
the count of ungrounded literals rises, against the run named in
`runs/latest`. The scorecard, with its delta from that run, is posted as a
comment on the pull request. After a nightly run on `main` passes,
`runs/latest` is updated by a pull request that commits the new run, so the
comparison point moves forward deliberately rather than on every push.
