...
[ABHA](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#abha) is the Ayushman Bharat Health Account, the patient half of [Registries](/docs/pr-17/docs/hiecm/v3/registries). It answers "who is this patient", and every record flow in [ABDM](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#abdm) starts from that answer.
...
| How it is issued   | After an Aadhaar based [KYC](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#kyc) check passes | Chosen by the person, or issued as a default               |
...
Store both. You match a patient record against the number, and you send the address when you link a [care context](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#care-context) or ask for consent. The number is issued only after a strong KYC process completes.
...
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------- | ---------------------- |
| Aadhaar [OTP](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#otp) | A code sent to the Aadhaar linked mobile number                                     | Mandatory           | Mandatory              |
...
- **Every number gets a default address**, the number with a suffix: `14digit@sbx` in [sandbox](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#sandbox), `14digit@abdm` in production. The M1 Postman collection shows a `preferredAbhaAddress` field holding the 14 digits with the `@abdm` suffix and no hyphens.
...
- **An address can exist without a number.** One can be created on the [HIE-CM](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#hie-cm) from mobile number, name, age and gender, self declared and with no KYC. Expect accounts with no number behind them.
...
The communication mobile number need not be the Aadhaar linked one. It is verified separately, by its own OTP, after enrolment. Email is optional throughout. An ABHA also carries a card, downloadable as an image, and a QR code, both M1 calls. Field level detail is on [M1 APIs](/docs/pr-17/docs/hiecm/v3/api/m1).
...
[M1](/docs/pr-17/docs/hiecm/v3/api/m1) is the only milestone that writes to this registry. It covers creation, login, profile management and sessions. Login by mobile number, Aadhaar number, ABHA number and ABHA address are all four mandatory for both private and government integrators.

## What every other milestone assumes

- **[M2](/docs/pr-17/docs/hiecm/v3/api/m2)** links a care context to the ABHA address and answers [discovery](/docs/pr-17/docs/hiecm/v3/getting-started/glossary#discovery) against it. See [Linking records](/docs/pr-17/docs/hiecm/v3/concepts/linking).
- **[M3](/docs/pr-17/docs/hiecm/v3/api/m3)** raises a consent request against the ABHA address. See [Consent](/docs/pr-17/docs/hiecm/v3/concepts/consent).
- **[PHR applications](/docs/pr-17/docs/hiecm/v3/concepts/phr)** sign a person in by ABHA address and show the records linked to it.

No flow starts without an ABHA, so M1 comes first even when your real goal is M2 or M3.

## Next

- [NHPR](/docs/pr-17/docs/hiecm/v3/registries/nhpr), the professional and facility registries.
- [M1 Create, ABHA Creation and Verification](/docs/pr-17/docs/hiecm/v3/api/m1), the guide.
- [M1 API reference](/docs/pr-17/reference/hiecm-m1).
- [Sandbox data dictionary](/docs/pr-17/docs/hiecm/v3/reference/data-dictionary), test identities.
