...
P2 is the mirror of [M2 Health Information Provider](/docs/pr-30/docs/hiecm/v3/milestones/m2). M2 is a provider publishing a record. P2 is the patient discovering it and linking it to their own [ABHA address](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#abha-address).

## In short

- Discovery is for facilities the user visited without giving an ABHA address, and for old records.
- A [HIP](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#hip) is expected to answer a discovery request within 10 seconds.
- Never show a [care context](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#care-context) that is already linked.
...
3. Calls the [HIE-CM](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#hie-cm) to share the details.
...
Counter names arrive in the QR code: 1 to 250 characters, letters, digits and spaces, with `.`, `-` and `_` allowed between them. A counter name cannot be the facility ID, the [HPID](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#hpid), the HIP ID or the HIP name.

## Discovery and user initiated linking

The user searches for the facility by name. Only participating facilities appear, and the facility must be a HIP linked to an [HRP](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#hrp). Your app sends a discovery request carrying the HIP ID and unverified identifiers of type `MR`, `MOBILE`, `ABHA_NUMBER` or `ABHA_ADDRESS`.

The user selects care contexts and confirms. The HIP sends an [OTP](/docs/pr-30/docs/hiecm/v3/getting-started/glossary#otp) to the registered mobile number, and on successful verification the care contexts link to the ABHA address.
...
An uploaded record is shareable once you hold three things: a link token from the generate link token call, a care context added by HIP initiated linking from [M2 Attach](/docs/pr-30/docs/hiecm/v3/milestones/m2), and the M2 health information transfer APIs.
...
- The calls and base URLs: [P2 API reference](/docs/pr-30/reference/hiecm-p2).
- The next milestone: [P3 Consent and notifications](/docs/pr-30/docs/hiecm/v3/milestones/p3).
