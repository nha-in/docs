---
id: nhcx.test.payer-uc-08
type: test
gateway: nhcx
milestone: n/a
version: nhcx-v1
title: 'Payer sandbox exit use case 8: Respond to the insurance plan request'
summary: >-
  Prove that your insurance system can answer a hospital's request for a policy's
  plan through the claims exchange.
sources:
- url: https://hcxsbx.abdm.gov.in/images/bd0c2ad1d5f672c40562.pdf
  ...
  fetched: '2026-09-14'
  note: NHCX Payer Side Use Cases- Sandbox Exit Process, row 10 of the NHCX document sheet, listed on https://hcxsbx.abdm.gov.in/#/documents. pages 2-3, Tables 2.3 and 3.1, Use case 8.
...
# Payer sandbox exit use case 8: Respond to the insurance plan request

## In plain words

A hospital asks, through [NHCX](../../shared/glossary/nhcx.md), for the plan behind a policy: what it covers, on what conditions, with which documents. This case proves your system answers with the plan, sealed for that hospital and addressed back to its request.
...
- A sandbox provider participant sends your payer a `POST /v1/insuranceplan/request` request. You need a provider partner, or a provider participant of your own, to send it.

## What happens

### Receive the request

1. Wait for `POST /v1/insuranceplan/request` on your registered endpoint.
...
1. If you processed the request, build a bundle holding the InsurancePlan resources for the policy. See [insurance plan bundles](../fhir/insurance-plan-bundle.md). Validate it against the NRCeS profiles ([validating a bundle](../fhir/validation.md)). Seal it as a [JWE](../glossary/jwe.md) with the provider's public key.
...
4. Send the answer to `/v1/insuranceplan/on_request`.
...
> Respond with the insurance plan details request via NHCX
>
...
- NHCX answers your `POST /v1/insuranceplan/on_request` with HTTP 202.
...
- The sealed payload is a bundle holding InsurancePlan resources, and it validates against the NRCeS profiles.
