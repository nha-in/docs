---
id: nhcx.test.payer-uc-09
type: test
gateway: nhcx
milestone: n/a
version: nhcx-v1
title: 'Payer sandbox exit use case 9: Respond to the preauthorization submitted'
summary: >-
  Prove that your insurance system can send its preauthorisation decision back to
  the hospital through the claims exchange.
sources:
- url: https://hcxsbx.abdm.gov.in/images/bd0c2ad1d5f672c40562.pdf
  ...
  fetched: '2026-09-14'
  note: NHCX Payer Side Use Cases- Sandbox Exit Process, row 10 of the NHCX document sheet, listed on https://hcxsbx.abdm.gov.in/#/documents. pages 3-4, Tables 3.2 and 4.1, Use case 9.
...
# Payer sandbox exit use case 9: Respond to the preauthorization submitted

## In plain words

A hospital submits a [preauthorisation](../glossary/preauthorisation.md) through [NHCX](../../shared/glossary/nhcx.md). This case proves your system sends back the adjudicated decision, sealed for that hospital and addressed back to its request.
...
- A sandbox provider participant sends your payer a `POST /v1/preauth/submit` request. You need a provider partner, or a provider participant of your own, to send it.

## What happens

### Receive the request

1. Wait for `POST /v1/preauth/submit` on your registered endpoint.
...
1. If you processed the request, build a ClaimResponse bundle with the adjudicated preauthorisation. See [the preauthorisation response bundle](../fhir/preauth-response.md). Validate it against the NRCeS profiles ([validating a bundle](../fhir/validation.md)). Seal it as a [JWE](../glossary/jwe.md) with the provider's public key.
...
4. Send the answer to `/v1/preauth/on_submit`.
...
> Payer to respond with API payload which should contain the adjudicated Preauthorization details.
>
...
- NHCX answers your `POST /v1/preauth/on_submit` with HTTP 202.
