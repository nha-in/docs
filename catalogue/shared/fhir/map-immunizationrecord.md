---
id: shared.fhir.map-immunizationrecord
type: fhir
gateway: shared
milestone: n/a
version: abdm-v3
title: ImmunizationRecord, the immunization record
summary: >
...
digests.json's ImmunizationRecord digest has an empty `sections` list:
**NRCES does not slice `Composition.section` for this record type.**
What is constrained is `Composition.section` itself (required, exactly
one) and `Composition.section.entry` (required, at least one, any
number).
...
"ImmunizationRecord"`) covers the shared document-bundle envelope and
