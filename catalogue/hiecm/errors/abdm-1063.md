...
sources:
  ...
  - file: catalogue/annexure/integration-learnings-2026-09-16.md
    ...
    note: >
      The patient narrowing the date range at approval. Observed by an integrator on 2026-09-16, not yet run from this repository.
related:
  flows: [hiecm.flow.m2-link-care-context, hiecm.flow.m3-fetch-records]
  concepts: [hiecm.concept.artefact-date-range]
skills:
  - hiecm-m2-debug
  - hiecm-m3-debug
  - abdm-errors
...
request inside it. The patient can narrow the range you asked for when
they approve, so build the request from `permission.dateRange` on the
fetched artefact, never from your own init request. See
