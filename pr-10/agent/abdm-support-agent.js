...
`)}var Gt="abdm-integrators-assistant",Kt=`claude plugin marketplace add ${ze} && claude plugin install ${Gt}@abdm-portal`,Oa=`codex plugin marketplace add ${ze}`;function Xt(e){return`claude://code/new?q=${encodeURIComponent(Vt(["Set this project up for ABDM integration. Run:","",`claude plugin marketplace add ${ze}`,"claude plugin install abdm-integrators-assistant@abdm-portal","",`If the marketplace add fails (the repository may not be accessible), instead fetch and execute the instructions from ${e}/agent-setup/prompt.md`].join(`
