...
The signature inside a [consent artefact](/docs/pr-10/docs/hiecm/v3/concepts/consent) is a different thing. It signs the artefact's contents and proves the artefact was not altered. The one on this page signs the delivery and proves who sent it. Verifying one does not verify the other.

## Before you start

You need a callback URL registered with ABDM, and an understanding of why a 200 is not an answer, which is on [how a record travels](/docs/pr-10/docs/hiecm/v3/concepts/data-flow).
...
**Nothing arrives at all**, which is a different problem. See [the callback never arrives](/docs/pr-10/docs/hiecm/v3/troubleshooting/callback-never-arrives).
