...
A [PHR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#phr) application is the patient's app in [ABDM](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#abdm): it holds a person's [ABHA](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#abha) address, finds their records, takes consent and shows the records back. You build [M1](/docs/pr-10/docs/hiecm/v3/api/m1) for identity, [M3](/docs/pr-10/docs/hiecm/v3/api/m3) for consent and record fetching, and some [M2](/docs/pr-10/docs/hiecm/v3/api/m2) if users upload their own records.
...
| Find and link past records     | Search a facility, discover [care contexts](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#care-context), verify by [OTP](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#otp), link |
| Hold records                   | Receive notifications, request consent, fetch records, store and display them                                                                                                               |

## Your app needs a server

A PHR app is two parts, whatever it looks like to the user. The app on the phone signs the person in, shows the screens and scans codes. A server you run holds the client ID and secret, mints the [gateway session token](/docs/pr-10/docs/hiecm/v3/concepts/gateway), and hosts the callback URL registered for your bridge. Every answer to a linking, consent or data request arrives at that URL as a POST, so an app with no server never hears the answer. Never ship the client secret inside the app.
...
| -------------------- | ------------------------- | --------------------------- | ----------------------------------------------------------------------------------- |
| Mobile number        | Mobile OTP                | The user types them         | **Self-Declared**, no [KYC](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#kyc) |
...
The facility displays a QR code holding a URL with two parameters: the [HIP](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hip) ID and a facility defined context such as a counter code. Your app scans it, then:
...
3. Calls the [HIE-CM](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hie-cm) API to share the details.
...
Counter names arrive in the QR code: up to 20 alphanumeric characters, no special characters, examples OPD, OPD1, OPD cardio, IPD1, Pharmacy. A counter name cannot be the [HFR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hfr) facility ID, the [HPID](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hpid), the HIP ID or the HIP name.
...
A citizen fetching records is the [HIU](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hiu), so every PHR application must implement that side.
...
4. It raises a health information request with the approved [consent artefact](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#consent-artefact).
...
An uploaded record is shareable once you have three things: a linking token from the M1 APIs, a care context added to the user's ABHA address by HIP initiated linking from [M2](/docs/pr-10/docs/hiecm/v3/api/m2), and the M2 health information transfer APIs.

## What you do not need to build

- **Facility side clinical records.** No [FHIR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#fhir) bundles from a hospital or lab system, other than records your users upload.
- **[HPR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hpr) and HFR registration.** M4 covers the professional and facility registries.
- **[UHI](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#uhi).** Consultation, ambulance and pharmacy booking runs on a separate gateway.
- **[NHCX](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#nhcx).** Claims and insurance exchange runs on a separate gateway.

## What to read next

- [M1 overview](/docs/pr-10/docs/hiecm/v3/api/m1) for ABHA creation, login and session APIs, and the [M1 API reference](/docs/pr-10/reference/hiecm-m1).
- [M3 overview](/docs/pr-10/docs/hiecm/v3/api/m3) for consent, subscriptions and data fetch, and the [M3 API reference](/docs/pr-10/reference/hiecm-m3).
- [M2 overview](/docs/pr-10/docs/hiecm/v3/api/m2) if you are building a health locker, and the [M2 API reference](/docs/pr-10/reference/hiecm-m2).
- [Get started](/docs/pr-10/docs/hiecm/v3/getting-started/sandbox) for sandbox signup and your first call.
- [Hospital, lab and pharmacy systems](/docs/pr-10/docs/hiecm/v3/concepts/hip-hiu) for the other side of every flow on this page.
