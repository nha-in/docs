...
Milestone 2 attaches the health records you hold to a person's [ABHA address](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#abha-address). After it you can group records into [care contexts](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#care-context), link them, answer [discovery](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#discovery) requests from [PHR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#phr) apps, and push encrypted records when a consented request arrives.

[Try the M2 APIs](/docs/pr-10/docs/hiecm/v3/api/m2)

[Every call in M2, one page each: the headers it needs, the payload it takes, the callback it triggers, and a request builder you can fire at the sandbox.](/docs/pr-10/docs/hiecm/v3/api/m2)

[Error codes](/docs/pr-10/docs/hiecm/v3/api/m2/errors)

[What each code M2 returns actually means, and the first thing to check when you see one.](/docs/pr-10/docs/hiecm/v3/api/m2/errors)

## In short

- Your facility is the [HIP](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hip) when it publishes a record, and your software is how it publishes. A valid facility ID and registration in the HIP role come first, either from the NHPR portal by hand or by building [M4 Enrol](/docs/pr-10/docs/hiecm/v3/milestones/m4).
- M2 is keyed to an ABHA address, so [M1 Create](/docs/pr-10/docs/hiecm/v3/milestones/m1) has to work before M2 can.
- Hold a [link token](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#link-token) per patient. Its validity is six months.
- Records go out as [FHIR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#fhir) R4 against the ABDM profiles.
...
Requesting records from other facilities is [M3 Retrieve](/docs/pr-10/docs/hiecm/v3/milestones/m3).

## Who needs it

Hospitals, laboratories, pharmacies and imaging centres, through the software they record care in. A citizen pushing their own records from a PHR app builds [P2 Linking and records](/docs/pr-10/docs/hiecm/v3/milestones/p2), the patient side of M2.

## Prerequisites

1. A valid facility ID and registration in the HIP role. That authorises your facility to create health records and share them with whoever asks to read them as the [HIU](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hiu). It comes from [M4 Enrol](/docs/pr-10/docs/hiecm/v3/milestones/m4).
2. A working [M1 Create](/docs/pr-10/docs/hiecm/v3/milestones/m1) integration. Linking is keyed to an ABHA address.
...
An [HMIS](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hmis) must implement every [HI type](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hi-type).
...
[SKILL.md](/docs/pr-10/skills/abdm-m2/SKILL.md "The router. Use the command below to take the references with it.")
...
`mkdir -p .claude/skills/abdm-m2/references && curl -fsSL https://nha-in.github.io/docs/pr-10/skills/abdm-m2/SKILL.md -o .claude/skills/abdm-m2/SKILL.md && for f in scaffold integrate debug test; do curl -fsSL https://nha-in.github.io/docs/pr-10/skills/abdm-m2/references/$f.md -o .claude/skills/abdm-m2/references/$f.md; done`

[Open in Claude](claude://code/new?q=Install%20the%20ABDM%20M2%20agent%20skill%20into%20this%20project%2C%20then%20help%20me%20use%20it.%0A%0ARun%20this%3A%0Amkdir%20-p%20.claude%2Fskills%2Fabdm-m2%2Freferences%20%26%26%20curl%20-fsSL%20https%3A%2F%2Fnha-in.github.io%2Fdocs%2Fpr-10%2Fskills%2Fabdm-m2%2FSKILL.md%20-o%20.claude%2Fskills%2Fabdm-m2%2FSKILL.md%20%26%26%20for%20f%20in%20scaffold%20integrate%20debug%20test%3B%20do%20curl%20-fsSL%20https%3A%2F%2Fnha-in.github.io%2Fdocs%2Fpr-10%2Fskills%2Fabdm-m2%2Freferences%2F%24f.md%20-o%20.claude%2Fskills%2Fabdm-m2%2Freferences%2F%24f.md%3B%20done%0A%0AIf%20this%20session%20did%20not%20open%20in%20the%20repository%20I%20am%20integrating%20ABDM%20into%2C%20ask%20me%20for%20the%20path%20before%20you%20write%20anything.)
...
M2 has no certification step of its own. One exit process covers the whole integration, run once, after every milestone your role needs works end to end. See [Going live](/docs/pr-10/docs/hiecm/v3/getting-started/going-live) for the four steps and what each one asks of you.

Test data is in the [data dictionary](/docs/pr-10/docs/hiecm/v3/reference/data-dictionary). [Support](/docs/pr-10/docs/support) lists the channels. The cases you are certified against are in [M2 testing use cases](/docs/pr-10/docs/hiecm/v3/resources/testing/m2).

## The journey, one diagram per flow

Milestone 2 (M2) of [ABDM](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#abdm) has four flows. This page draws each one, so you can see the round trips before you read the detail.

"Your system" is the [HMIS](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hmis) or [LMIS](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#lmis) your facility publishes through. "HIE-CM" is the [Health Information Exchange and Consent Manager](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hie-cm) gateway.
...
The patient gave you their [ABHA address](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#abha-address) at registration. Link the [care context](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#care-context) as soon as the record is ready: linking is what makes it reachable from [PHR](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#phr) apps.

Step 8 is the acknowledgement. Step 9 is the answer. The link is confirmed only when the callback arrives at `/v3/link/on_carecontext` on your bridge carrying `status`, so do not mark a record as linked on the strength of the 202. See [the outcome of a care context linking call](/docs/pr-10/docs/hiecm/v3/api/m2/endpoints/m2-on-carecontext-result).
...
- **No valid [link token](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#link-token).** Validate the stored token before use, with a tool such as JWT.io. If it is expired or missing, regenerate it through demographic authentication.
...
The patient starts [discovery](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#discovery) from a PHR app and picks the facility they visited. Your system matches them on the identifiers the gateway passes you and answers with care contexts.
...
Another facility, an insurer or a citizen's PHR app asks for records under a [consent artefact](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#consent-artefact) the patient granted. Whoever asks is the [HIU](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#hiu).
...
Encryption uses [ECDH](/docs/pr-10/docs/hiecm/v3/getting-started/glossary#ecdh), Elliptic Curve Diffie Hellman, over Curve25519. Mechanics: [use cases](/docs/pr-10/reference/hiecm-m2). Call order: [API reference](/docs/pr-10/reference/hiecm-m2).

## Next

- The four flows as diagrams: [the journey below](#the-journey-one-diagram-per-flow).
- The calls, callbacks and error codes: [M2 API reference](/docs/pr-10/docs/hiecm/v3/api/m2).
- The next milestone: [M3 Retrieve](/docs/pr-10/docs/hiecm/v3/milestones/m3).
