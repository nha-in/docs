...
goes. There is no shared test Aadhaar number, so do not go looking for one in
these pages.

## What this page does with what you type

The calls below go straight from your browser to the ABDM hosts. There is no proxy and this
site has no server of its own, so nothing you type is sent anywhere else and nothing is
written to a URL.

- The **Aadhaar number** is encrypted in your browser with the ABDM public key before it
