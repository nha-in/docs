...
- **Compiled** (`hiecm-<module>-build` and `hiecm-<module>-debug`, one
  ...
  `subscription` and `scan-and-pay`): assembled from Catalogue atoms by
  ...
  not reached yet ships nothing rather than an empty loop, and a retired
  module's folder is removed rather than left behind.
