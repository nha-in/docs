---
name: hiecm-p2-build
description: "Use when scaffolding an integration against ABDM P2 (the PHR profile, linking an ABHA number, switching profiles, and linking, sharing and consent for the patient): builds each journey as an observe-orient-decide-act loop against the sandbox."
---
# HIE-CM p2 build
...
```bash
curl --request POST \
  --url https://dev.abdm.gov.in/api/hiecm/consent/v3/request/{consentRequestId}/deny \
  --header 'Authorization: Bearer <ACCESS_TOKEN_FROM_SESSIONS_CALL>' \
  --header 'REQUEST-ID: 18235d89-cb13-479d-ad71-7a57d5f669a8' \
  --header 'TIMESTAMP: 2022-10-06T15:10:00.587Z' \
  --header 'X-CM-ID: sbx' \
...
#### 9. Request artefact by consent REQUEST-ID (`p2_get_consent_v3_artefact_request_request_id`)

```bash
curl --request GET \
  --url https://dev.abdm.gov.in/api/hiecm/consent/v3/artefact/request/{consentRequestId} \
  --header 'Authorization: Bearer <ACCESS_TOKEN_FROM_SESSIONS_CALL>' \
  --header 'REQUEST-ID: 18235d89-cb13-479d-ad71-7a57d5f669a8' \
  --header 'TIMESTAMP: 2022-10-06T15:10:00.587Z' \
  --header 'X-CM-ID: sbx' \
