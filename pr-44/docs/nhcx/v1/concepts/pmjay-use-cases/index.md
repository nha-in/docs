...
Envelope errors, receipts, retries and the error endpoint are the same as on any NHCX integration and are covered in [The JWE message format](/docs/pr-44/docs/nhcx/v1/getting-started/jwe-message-format). Two things are specific here. Every callback must accept the answer in two forms, decided by the `type` field. A sealed bundle when the payer processed the request, or a protocol response when it could not be opened or failed validation. And the PMJAY payer's own error codes, the `PAYR-12xx` set for preauthorisation and `PAYR-13xx` for claims, name the exact field or rule that failed; surface them to the user as they are.
