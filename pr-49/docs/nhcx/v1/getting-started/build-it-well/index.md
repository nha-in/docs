...
Do not match a `PAYR` code on its number alone. The same number means different things from different payers. Read the code with the message text beside it, and log both. [Reading error codes](/docs/pr-49/docs/nhcx/v1/reference/error-code-guide) lists the collisions, and [error codes](/docs/pr-49/docs/nhcx/v1/reference/error-codes) lists every code.
...
Store the ids the transport returned, not the ones you meant to send. See [responses arrive against the wrong request](/docs/pr-49/docs/nhcx/v1/troubleshooting/responses-arrive-against-the-wrong-request).
...
[Receiving a callback](/docs/pr-49/docs/nhcx/v1/getting-started/receiving-a-callback) has the route and the receipt.
...
Then ask for the status rather than sending again: [status and search](/docs/pr-49/docs/nhcx/v1/getting-started/status-and-search).
...
- [Build with AI](/docs/pr-49/docs/nhcx/v1/getting-started/build-with-ai) sets an agent up with all seven skills and the documentation server.
- [Building and sending a JWE](/docs/pr-49/docs/nhcx/v1/getting-started/building-and-sending-a-jwe) is the seal every rule above assumes.
- [The provider checklist](/docs/pr-49/docs/nhcx/v1/roles/provider/provider-checklist) is what has to be in place before go live.
