...
One skill per module, covering the whole module: scaffolding, integrating and
debugging. An agent hits all three inside one session, so splitting them across
skills only means it loads all three.

The skill is generated by `scripts/build-skills.mjs` from the specifications
and the journey files. The capability counts on the page
