...
*Source: `documents/Standard Error Codes.xlsx` — all sheets*


## Sheet: Payer Error Codes

| Error Codes | Error Description | Type of error | Internal Usecases |
|---|---|---|---|
| PAYR-1001 | Error while decrypting request. Please reprocess the request encrypting with valid certificate of the payer. | Transport |  |
| PAYR-1002 | Error while encrypting request. Please update encryption certificate in NHCX. | Transport |  |
| PAYR-1003 | Error while connecting to NHCX. System will try to resend the message to NHCX after some time. | Transport |  |
| PAYR-1004 | Provider is not registered with the payer for requested policy. | Business | Coverage Eligibility, Preauthorization, Claim |
| PAYR-1005 | Beneficiary is not a covered member for requested policy. Please enroll beneficiary for the policy and try again. | Business | Coverage Eligibility, Preauthorization, Claim |
| PAYR-1006 | Policy does not exist. Please try with a valid policy. | Business | Coverage Eligibility, Preauthorization, Claim |
| PAYR-1007 | Requested policy is expired. Please try with a valid policy. | Business | Coverage Eligibility, Preauthorization, Claim |
| PAYR-1008 | Eligible coverage amount of the beneficiary for the requested policy is insufficient to process the request. | Business | Preauthorization, Claim |
| PAYR-1009 | Requested items are not valid or not covered. Please try again with valid or covered items. | Business | Preauthorization, Claim |
| PAYR-1010 | Preauthorization is required to process the request but was not obtained. | Business | Preauthorization (Enhancement, Resubmission), Claim |
| PAYR-1011 | Enhancement request can not be processed as the prior approved package/procedure does not support enhancement. | Business | Preauthorization (Enhancement) |
| PAYR-1012 | Claim amount requested is more than the preauthorization approved amount. | Business | Claim |
| PAYR-1013 | No prior approval for the requested procedures/packages obtained. | Business | Preauthorization (Enhancement), Claim |
| PAYR-1014 | Date of birth of the beneficiary can not be after date of service. | Business | Coverage Eligibility, Preauthorization, Claim |
| PAYR-1015 | Date of service cannot be after date of death. | Business | Claim |
| PAYR-1016 | Duplicate claim submitted based on service codes and dates. | Business | Claim |
| PAYR-1017 | Amount calculations are not correct. Please verify calculated amount. | Business | Preauthorization, Claim |
| PAYR-1018 | Time limit for submission expired. | Business | Preauthorization (Enhancement, Resubmission), Claim |
| PAYR-1019 | Requested additional information was not received or was not received within time limit | Business | Preauthorization (Query), Claim (Query) |
| PAYR-1020 | Unable to process payment as no valid bank details found for the provider. | Business | Payment |

## Sheet: NHCX Error Codes
...
|---|---|---|---|
| PAYR-1001 | Error occurred while decrypting payload for receiver code <receiver_code> with correlation id <sender_correlation_id>. | This error comes during processing the request apis if receiver system is unable to decrypt the received encrypted payload using the private key of the receiver. | 1. Please validate if the receiver is a valid registered participant in NHCX.<br>2. Validate if the request is encrypted with the public certificate available in NHCX for the receiver.<br>3. Re-fetch the public certificate of the receiver from NHCX and re-send the request.<br>4. Validate the format of the payload before encrypting. Payload should be in key-value format ensuring each of the below given values are present:<br><br>x-hcx-sender_code = participant_id of the sender from NHCX<br>x-hcx-recipient_code = participant_id of the receiver from NHCX<br>x-hcx-api_call_id = api_caller_id which is UUID<br>x-hcx-correlation_id = sender_correlation_id which is UUID<br>x-hcx-workflow_id = applicable workflow id mentioned in the workflow sheet on website<br>x-hcx-timestamp = UTC timestamp for the current time in seconds. e.g. - 1773995681<br>x-hcx-status = values should be within (request.initiated, request.queued) for initiating request (calling request/submit api); or values should be within (response.complete, response.error, response.partial) for initiating response (calling on-request/on-submit api)<br>payload = encrypted FHIR bundle<br><br>5. If error is not resolved following the above steps, connect with NHCX support team. |
| PAYR-1002 | Error occurred while encrypting payload for receiver code <receiver_code> with correlation id <sender_correlation_id>. | This error comes while request is successfully received and processed by the payer. But encounters error while encrypting the response to be sent to the provider,using the public key available in NHCX for the provider. | 1. Validate if the hospital is a valid registered participant in NHCX.<br>2. Update the public key used for encryption in the NHCX portal. The payer system will fetch the updated public key from NHCX and will send the response.<br>3. If error is not resolved following the above steps, connect with NHCX support team.<br> |
| PAYR-1003 | Invalid workflow requested. Hence request will not be processed further. | Depricated |  |
| PAYR-1004 | Received FHIR bundle is malformed. Please correct the bundle and try again. <Error details> | This error occurs while pushing the data to health lake where the bundle structure gets validated. From health lake if status is received as 400, then this error comes. | 1. Check the error details and make the rectifications accordingly.<br>2. Validate the updated FHIR bundle in NRCES validator.<br>3. If error is not resolved following the above steps, connect with NHCX support team. |
| PAYR-1005 | Maximum time limit exceeded in receiving the request. Please try again. | This error comes while processing the request from the provider or sender when the timestamp coming in the request header as the value of x-hcx-timestamp (refer to error code PAYR-1001 for expected request header details), exceeds the current time by 24 hours. | 1. Ensure x-hcx-timestamp has the value which not more than 24 hours before the current time.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
| PAYR-1006 | Invalid name in request. Hence request will not be processed further. | Depricated |  |
| PAYR-1007 | Invalid gender in request. Hence request will not be processed further. | Depricated |  |
| PAYR-1008 | Invalid FHIR bundle received. Hence request will not be processed further. | This error occurs if the the received FHIR bundle is not valid or can not be parsed. | 1. Validate the FHIR bundle in NRCES validator.<br>2. Check the error details and make the rectifications accordingly.<br>3. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid input, code and reason code received. Please try again with valid combination. To get the valid combination for task request, please refer to the document. | This error occurs while processing the task request. If the input, code and reason code are not provided according to the applicable combination, system does not accept the request. | 1. Ensure the input, code and reason code (refer to NRCES website) are according to the applicable combination given in the integration guide.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid case number received. Please try again with valid case number. | This error occurs if the FHIR bundle does not contains a valid reference number. This will be the provider's internal reference number or the preauth reference number provided by payer. This value is mandatory for all the scenarios. | 1. For task request:<br>    a. For any request related to preauthorization, value of reference should be sent as "initimationNumber" as input parameter.<br>    B. For any request related to preauthorization, value of reference should be sent as "claimNumber" as input parameter.<br><br>2. For any other request, this vaue should be sent as part of the identifier of the parent resource.<br>3. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid Base64 value received in attachment. Please try again with valid Base64 value. Please refer to https://hl7.org/fhir/R4/datatypes.html#Attachment | This error occurs when value of attachment in FHIR bundle is not Base64 encoded string. | 1. Ensure the value of the attachment is Base64 encoded.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid attachment name received in attachment. Please try again with valid name. | This error occurs when null/empty value is sent as the name of attachment in FHIR bundle. | 1. Valid name must be sent as the attachment name for the attachments.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid content type received in attachment. Please try again with valid name. Applicable content types are - application/pdf, application/jpg, application/jpeg, application/png, application/fhir+json. For further information please refer to NHCX Integration Handbook document. | This error occurs when null/empty/unacceptable value is sent as the content type of attachment in FHIR bundle. | 1. Content type must not be null/empty/unacceptable value.<br>2. Applicable content types are - application/pdf, application/jpg, application/jpeg, application/png, application/fhir+json<br>3. If error is not resolved following the above steps, connect with NHCX support team. |
|  | No user role found/associated for sender code %s in NHCX system. Please try again with valid sender details. | This error occurs when sender is not associated with any role in NHCX system. | 1. Sender must be associated with a role in NHCX system.<br>2. Connect with NHCX support team for further support. |
|  | Invalid HFR Id received. Please try again with valid HFR details. | This error occurs when HFR id is not sent in the organization resource in FHIR bundle. | 1. In the organization resource, HFR id value should be sent as an identifier with type code as "NPI".<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | HFR Id in the request does not match with the associated registry id in NHCX. Please try again with valid HFR details. Please ensure the HFR id sent in the FHIR request must match with the registry id specified in NHCX for the sender. | This error occurs when HFR id sent in the organization resource in FHIR bundle does not match with the registry id specified in NHCX for the participant/sender. | 1. HFR id sent in the FHIR must match with registry id updated in NHCX for the participant.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid Base64 value received in attachment for item <item code>. Please try again with valid Base64 value. Please refer to https://hl7.org/fhir/R4/datatypes.html#Attachment | This error occurs when value of attachment in FHIR bundle is not Base64 encoded string for an item in preauth /claim request. | 1. Ensure the value of the attachment is Base64 encoded for an item in preauth /claim request.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid attachment name received in attachment for item %s. Please try again with valid name. | This error occurs when null/empty value is sent as the name of attachment in FHIR bundle for an item in preauth /claim request. | 1. Valid name must be sent as the attachment name for the attachments  for an item in preauth /claim request.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
|  | Invalid content type received in attachment for item %s. Please try again with valid content type. Applicable content types are - application/pdf, application/jpg, application/jpeg, application/png, application/fhir+json. For further information please refer to NHCX Integration Handbook document. | This error occurs when null/empty/unacceptable value is sent as the content type of attachment in FHIR bundle for an item in preauth /claim request. | 1. Content type must not be null/empty/unacceptable value.<br>2. Applicable content types are - application/pdf, application/jpg, application/jpeg, application/png, application/fhir+json<br>3. If error is not resolved following the above steps, connect with NHCX support team. |
...
| PAYR-1009 | No identifier found for patient component. Hence request will not be processed further. https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Patient.html | This error occurs if identifier is missing in the patient resource in the FHIR bundle. | 1. Ensure identifier is present for patient resource of FHIR bundle.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
| PAYR-1010 | No type found for patient component identifier. Hence request will not be processed further. https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Patient.html | This error occurs if type is missing in the identifier of the patient resource in the FHIR bundle. | 1. Type value needs to be sent in the identifier of any resource of FHIR bundle.<br>2. If error is not resolved following the above steps, connect with NHCX support team. |
...
| PAYR-1093 | No subject found for encounter in composition component. Hence request will not be processed further. Please refer to https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Encounter.html | This error comes when composition resource structure does not adhere to NRCES guidelines. | Please refer to https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Encounter.html |
| PAYR-1094 | Invalid subject type found for encounter in composition/observation component. Hence request will not be processed further. Please refer to https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Encounter.html | This error comes when invalid subject type  found for composition resource structure does not adhere to NRCES guidelines. | Please refer to https://www.nrces.in/ndhm/fhir/r4/StructureDefinition-Encounter.html |
| PAYR-1095 | Invalid discharge information received for claim request. Hence request will not be processed further. Please send the discharge information in supporting info in claim resource, with category as DIS (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-category.html), and codes in LAMA/DAMA/DTH/DTM (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-code.html). | This error comes when no discharge information in received in claim request FHIR bundle. | Please send the discharge information in supporting info in claim resource, with category as DIS (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-category.html), and codes in LAMA/DAMA/DTH/DTM (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-code.html) |
| PAYR-1096 | Invalid death date received for claim request. Hence request will not be processed further. Please send the death date information in supporting info in claim resource, with category as ONS (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-category.html), and code as DTM (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-code.html). | This error comes when no death date is received in claim request FHIR bundle for discharge type as death. When discharge type is death, death date is mandatory to be sent. | Please send the death date information in supporting info in claim resource, with category as ONS (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-category.html), and code as DTM (refer - https://www.nrces.in/ndhm/fhir/r4/ValueSet-ndhm-supportinginfo-code.html). |
| PAYR-1097 | No payload found in the request. Please ensure that the request that is being sent, contains encrypted payload within the mandatory payload properties. | This error comes when no payload is received in the request during decryption of the request. For more information, please google "in com.nimbusds.jose.JWEObject which part has the payload". | In mandatory payload properties, in ciphertext, there must be the required encrypted payload. |
...
| PAYR-1252 | Case number %s is not in active preauthorization state with the current status of the case with the payer system is %s. Hence the preauthorization can not be cancelled. Only the cases with current status as preauthorization submitted or preauthorization approved can be cancelled. | This error occurs when the system receives the preauth cancel request for a case number which is not currently in preauthorization submitted or preauthorization approved state. | Participant cannot cancel the case. |
| PAYR-1253 | Case number %s is already cancelled. Hence the preauthorization can not be cancelled again. Only the cases with current status as preauthorization submitted or preauthorization approved can be cancelled. | This error occurs when the system receives the preauth cancel request for a case number which is already cancelled. | Participant cannot cancel the case. |
| PAYR-1254 | Response for STG Questionnaire id %s is mandatory for procedure code %s. Hence the preauthorization request will not be processed as the questionnaire response is not received for procedure code. | This error occurs when the STG is not passed for a particular procedure in the preauth request. | STGs should be passed as questionnaire response with category code as STG, included in supporting info in claim resource. Value for this supporting info item will Refrence. And with that reference url, there should be a Questionnaire Response resource for the STG. |
| PAYR-1255 | Case number %s is already cancelled. Hence no preauthorization request will be accepted for this case number. New preauthorization request needs to be raised with new case/reference number to proceed further. | This error occurs when the system receives a preauth request for a case number for which preauthorization request is already cancelled. | Preauthorization request can not be raised for any case number which is already cancelled. |
