...
- The [security audit](/docs/main/docs/hiecm/v3/getting-started/security-audit) is separate from functional testing, and both feed the exit form.
- Production credentials are issued at the end. They are not your sandbox values.

## Prerequisites

Every milestone your integration needs works end to end. [Your integration path](/docs/main/docs/hiecm/v3/milestones) says which ones apply to a citizen using a [PHR](/docs/main/docs/hiecm/v3/getting-started/glossary#phr) application, and which to a facility publishing as the [HIP](/docs/main/docs/hiecm/v3/getting-started/glossary#hip) or fetching as the [HIU](/docs/main/docs/hiecm/v3/getting-started/glossary#hiu).

Start the exit process once all of them are complete, not milestone by milestone.

## 1. Demonstrate what you built

You demonstrate the [ABDM](/docs/main/docs/hiecm/v3/getting-started/glossary#abdm) functionality you built to the integration team. This is the first of two demonstrations, and it opens the process rather than closing it.
...
Once the earlier steps are complete, a demonstration is scheduled for the [Health Tech Committee](/docs/main/docs/hiecm/v3/getting-started/glossary#health-tech-committee). Different audience, different demonstration. The committee records its decision in four review stages, each carrying its own reviewer and date, so the outcome arrives as a sequence rather than a single answer.
...
If a call that worked in the sandbox fails in production, check the base URL and the `X-CM-ID` header first. See [Everything returns 401](/docs/main/docs/hiecm/v3/troubleshooting/everything-returns-401).

Questions about the exit process itself, including where to submit the form or what counts as a valid supporting document, go to [Support](/docs/main/docs/support).

## Next steps

- Get the audit that feeds step 2: [Security audit](/docs/main/docs/hiecm/v3/getting-started/security-audit).
- Hand your integration to an agent: [Build with AI](/docs/main/docs/hiecm/v3/getting-started/build-with-ai).
