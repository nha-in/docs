...
In [M1](/docs/main/docs/hiecm/v3/api/m1) the answer comes back in the response to your call. In [M2](/docs/main/docs/hiecm/v3/api/m2) and [M3](/docs/main/docs/hiecm/v3/api/m3) it does not. The response only acknowledges your request, and the answer arrives afterwards as a POST to a URL you registered.

Register one base URL. We post to paths under it, and each callback that answers a call carries the `REQUEST-ID` you sent on it in `response.requestId`, so you can match the answer to the question. Each path is named on the page of the call it belongs to, in the [API reference](/docs/main/docs/hiecm/v3/api).
...
[Make your first call](/docs/main/docs/hiecm/v3/getting-started/first-fifteen-minutes).
