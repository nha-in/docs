...
You made a call, it came back with a 202 or a 200, and nothing else has happened since. This is a common report in [HIE-CM](/docs/main/docs/hiecm/v3/getting-started/glossary#hie-cm) integration. The checks below are in the order we recommend, not a record of how often each has turned out to be the actual cause.

That early response only means the [NHA](/docs/main/docs/hiecm/v3/getting-started/glossary#nha) gateway accepted your request. In M2 and M3 the real answer arrives later, as a POST from the gateway to a URL you registered in advance. See [the gateway](/docs/main/docs/hiecm/v3/concepts/gateway) for why this is the normal shape of these flows, and the [API reference](/docs/main/docs/hiecm/v3/api), where each call names the callback it produces.

## Work through these in order

1. **Is the callback URL registered with the gateway?** Confirm it with the [update bridge callback URL](/docs/main/docs/hiecm/v3/api/gateway/endpoints/gateway-abdm-gateway/03-gateway-patch-gateway-v3-bridge-url) call. Setting a URL in a console once is not the same as confirming the gateway has it.
...
[Next Still stuck? Check the M2 user journey Confirm which step of the sequence your call sits in, so you know which callback should follow it.](/docs/main/docs/hiecm/v3/milestones/m2)
