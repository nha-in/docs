...
// The NHCX skills: well formed, stamped, and not drifting apart.
...
for (const slug of nhcxSlugs) {
  ...
  // Two things in these folders were written here and not upstream: the
  // scaffold, and the router's summary that points at it. A fresh copy from
  // github.com/nha-in/nhcx-skills brings neither, and would remove both
  // without a word. Failing here is what turns that into a decision.
  const scaffoldFile = join(nhcxDir, slug, "references", "scaffold.md");
  if (!existsSync(scaffoldFile)) {
    fail(slug, "references/scaffold.md is missing. A re-copy from upstream drops it: restore it, or push it upstream first");
  } else if (!/Loop limit: \d+ passes? per stage/.test(readFileSync(scaffoldFile, "utf8"))) {
    fail(slug, "references/scaffold.md states no loop limit");
  }
  if (!m[2].includes("## What is in this folder")) {
    fail(slug, "the router has no \"What is in this folder\" summary. A re-copy from upstream drops it");
  }
  // The em dash rule is repository wide, and these folders are 58 files each.
  for (const [rel] of folderFiles(join(nhcxDir, slug))) {
    if (!/\.(md|json|sh)$/.test(rel)) continue;
    if (readFileSync(join(nhcxDir, slug, rel), "utf8").includes("\u2014")) fail(slug, `em dash found in ${rel}`);
  }
}

// Every file a folder shares with the others has to be the same file. SKILL.md
// is what makes a folder its own use case, so it is the one exemption.
if (nhcxSlugs.length > 1) {
  const [first, ...rest] = nhcxSlugs;
  const baseline = folderFiles(join(nhcxDir, first));
  for (const slug of rest) {
    const theirs = folderFiles(join(nhcxDir, slug));
    for (const [rel, hash] of baseline) {
      if (rel === "SKILL.md") continue;
      if (!theirs.has(rel)) fail(slug, `shared file ${rel} is in ${first} and missing here`);
      else if (theirs.get(rel) !== hash) fail(slug, `shared file ${rel} differs from ${first}`);
    }
    for (const [rel] of theirs) {
      if (rel !== "SKILL.md" && !baseline.has(rel)) fail(slug, `file ${rel} is here and missing from ${first}`);
    }
  }
}
...
console.log(`All compiled skills trace back to the Catalogue. ${nhcxSlugs.length} NHCX skill(s) are well formed, stamped and identical where they overlap.`);
