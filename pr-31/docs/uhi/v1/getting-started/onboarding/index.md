...
Every [UHI](/docs/pr-31/docs/uhi/v1/getting-started/glossary#uhi) service document repeats the same joining route. After this page you will know the seven steps from the [M2](/docs/pr-31/docs/hiecm/v3/getting-started/glossary#m2) gate to production go-live, and where one service differs from the rest.

Read [Introduction](/docs/pr-31/docs/uhi/v1) and [UHI gateway](/docs/pr-31/docs/uhi/v1/concepts/network-and-protocol) first, for the network, the two roles and the message pattern.

## Pick your role first

| Role                                                          | Full name                           | What you build                                                                                                                 |
| ------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| [EUA](/docs/pr-31/docs/uhi/v1/getting-started/glossary#eua)   | End User Application                | The citizen facing app. Sends the search, receives results at your callback URL, drives booking where the service supports it. |
| [HSPA](/docs/pr-31/docs/uhi/v1/getting-started/glossary#hspa) | Health Service Provider Application | The provider platform. Answers searches from its own catalogue and, where the service supports it, runs the booking lifecycle. |

The HSP is the hospital, clinic, doctor, ambulance operator or blood bank, and the HSPA is its digital interface. The [gateway](/docs/pr-31/docs/uhi/v1/getting-started/glossary#gateway) is the network's routing layer.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ----- | ----------------------------------------------------------- |
| [Physical consultation](/docs/pr-31/docs/uhi/v1/concepts/services/physical-consultation)                                                                                           | Yes | Yes   | Any onboarded provider platform                             |
| [Ambulance booking](/docs/pr-31/docs/uhi/v1/concepts/services/ambulance-booking)                                                                                                   | Yes | Yes   | Any onboarded ambulance operator platform                   |
| [Blood bank](/docs/pr-31/docs/uhi/v1/concepts/services/blood-bank)                                                                                                                 | Yes | Yes   | Open to both roles. e-RaktKosh is the registered HSPA today |
| [PM-JAY HEM](/docs/pr-31/docs/uhi/v1/concepts/services/pmjay-hem)                                                                                                                  | Yes | No    | NHA. There are no third party HSPA integrations             |
| [Jan Aushadhi Kendra](/docs/pr-31/docs/uhi/v1/concepts/services/jan-aushadhi-kendra) and [medicine search](/docs/pr-31/docs/uhi/v1/concepts/services/jan-aushadhi-medicine-search) | Yes | Named | PMBI                                                        |
| [AMRIT pharmacy](/docs/pr-31/docs/uhi/v1/concepts/services/amrit-pharmacy)                                                                                                         | Yes | Named | HLL Lifecare Limited                                        |
...
Your application must have completed ABDM M2 with [HIE-CM](/docs/pr-31/docs/uhi/v1/getting-started/glossary#hie-cm). An application that has not completed M2 cannot be onboarded onto UHI services. The gate applies to every service on the network.

If M2 is not done, start at [M2 Health Information Provider, Health Information Provider Services](/docs/pr-31/docs/hiecm/v3/api/m2).
...
Every UHI call is signed with an Ed25519 signature over a BLAKE-512 hash of the request body. The header format is on [UHI gateway](/docs/pr-31/docs/uhi/v1/concepts/network-and-protocol#signing). Onboarding needs the key pair.
...
```text
Gateway base URI        https://uhigatewaysandbox.abdm.gov.inReference EUA           http://uhieuasandbox.abdm.gov.in/api/v1/euaServiceReference HSPA          https://hspasbx.abdm.gov.in/api/v1/hspaProduction gateway      https://uhigateway.abdm.gov.in
```
...
For anything about these pages rather than about UHI itself, see [support](/docs/pr-31/docs/support).

## Next

- [UHI gateway](/docs/pr-31/docs/uhi/v1/concepts/network-and-protocol), for the message pattern, the context block and signing
- [Introduction](/docs/pr-31/docs/uhi/v1), for the two roles and how far each service goes
- [Physical consultation](/docs/pr-31/docs/uhi/v1/concepts/services/physical-consultation), the service with the full booking lifecycle
- [M2 Health Information Provider, Health Information Provider Services](/docs/pr-31/docs/hiecm/v3/api/m2), the prerequisite
