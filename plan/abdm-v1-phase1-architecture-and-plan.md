...
What is verified today, which is a smaller and different claim: 319 atoms are indexed, 311 carrying `status: unverified`, 4 carrying `status: draft` and 4 carrying `status: verified`. By gateway that is 251 HIE-CM atoms and 68 shared atoms. By milestone across the whole Catalogue it is 73 M1, 48 M2, 18 M3, 20 M4, 35 P1, 18 P2, 37 P3 and 70 `n/a`, the 70 being the 68 shared atoms, which lint requires to carry `n/a`, plus the 2 HIE-CM decision atoms. Those eight numbers sum to 319, which is the check. The four verified atoms are `m1-encrypt-value`, `m1-get-public-certificate`, `abdm-1016` and the shared `timestamp-header`, so three of the four are M1 and every other milestone has none. There are no PHR application services atoms, no UHI atoms and no NHCX atoms. Nothing in the paragraph above is verified unless it is counted in this one.
...
|---|---|---|---|
| HIE-CM (ABDM V3) M1, M2, M3 | **Atoms written** | `hiecm-m1.yaml`, `hiecm-m2.yaml` and `hiecm-m3.yaml`, 44, 10 and 7 operations. 139 atoms across the three milestones. 3 of them are verified, all 3 on M1. The other 136 are unverified. | All five dummy-proof sections on every atom. Every endpoint curl verified against sandbox. Every NHA functional test case is an atom. All nine skills compile and pass validation. First-day developer test passes for M1 (§9). |
| HIE-CM M4 (HPR, HFR, bridge linkage) | **Atoms written** | `hiecm-m4.yaml`, 2 operations, the two NHA gave a method and a path for. 6 generated pages under `api/m4/`. 20 atoms, all unverified. `abdm-m4` compiles. | Atoms carry the registration order, the identifier formats, the recorded errors and NHA's test cases. One page maps every other M4 call NHA describes inside a screenshot, marked a map rather than a build guide. No M4 call has been made against the sandbox, so no M4 atom is verified. |
| HIE-CM PHR modules P1, P2, P3 | **Atoms written** | `hiecm-p1.yaml`, `hiecm-p2.yaml` and `hiecm-p3.yaml`, 63, 49 and 35 operations. 156 generated pages. 90 atoms, all unverified. `abdm-p1`, `abdm-p2` and `abdm-p3` compile. | Atoms carry the patient-side flows, the wording NHA specifies for each outcome, and the AS error codes NHA records once for the whole patient side. No operation in them has been called against the sandbox. |
| HIE-CM PHR application services | Phase 2, no atoms | `hiecm-phr-services.yaml`, 61 operations. 64 generated pages. Zero atoms. `abdm-phr-services` compiles from the specification and its recorded rules, not from atoms. | The specification and its generated reference pages exist and are unverified. Nothing here is required to certify. |
