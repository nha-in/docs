...
When: inside the A10 poll, after A10 found no reply. The whole loop is in A10P; the parts that call this endpoint are below. Rows written: `claim` ([D9](../database/D9-claim.md)), `claim_plan` (D10 (in nhcx-preauth)), `claim_auth` ([D13](../database/D13-claim-auth.md)), `claim_preauth` (D18 (in nhcx-preauth)), `claim_predetermination` (D19 (in nhcx-preauth)), `claim_submission` (D20 (in nhcx-preauth)), `claim_enquiry` (D29 (in nhcx-preauth)).
...
- APIs: [A2. Coverage Eligibility Check](A2-coverage-eligibility-check.md), [A10. Transaction Related](A10-txn-related.md), [A13. Transaction List](A13-txn-list.md), [A17. Claim State](A17-claim-state.md)
