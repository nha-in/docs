...
A health record your system creates is invisible to [ABDM](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#abdm) until you link it, which means telling the [HIE-CM](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#hie-cm) that a named group of records exists at your facility for a named person. Until that happens no patient can discover the record and no consent request can reach it; the calls that do it are in the [M2 guide](/docs/pr-41/docs/hiecm/v3/api/m2).

## The unit that gets linked is a care context

You do not link a record. You link a [care context](/docs/pr-41/docs/hiecm/v3/concepts/care-context), a logical grouping of a patient's health records that your system defines. It carries a reference ID and a display name, and nothing else. What each field holds, and how to structure care contexts per visit and per admission, is on [Care contexts](/docs/pr-41/docs/hiecm/v3/concepts/care-context).

## Why the record has to be linked first

The HIE-CM is data blind. It holds a map: which [ABHA address](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#abha-address) has care contexts at which facilities. Linking puts an entry on that map, and everything downstream reads it.

- A [PHR](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#phr) app shows a patient their records by reading their linked care contexts.
...
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------- |
| [HIP](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#hip) initiated linking    | The patient shared their ABHA address with you                      | You                       |
| Notification to mobile                                                             | You hold a mobile number, name, age and gender, but no ABHA address | You, and then the patient |
| [Discovery](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#discovery) and link | The patient goes looking for old records from their PHR app         | The patient               |
...
**Discovery and link.** The request comes to you. The patient picks the facility they visited in their PHR app, and the HIE-CM forwards a discovery request to the [HRP](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#hrp) or HIP behind it. You match against your own patients and reply with care contexts, and the patient picks which to link. Implementing discovery is mandatory for every HIP, even if every patient gives you an ABHA address at the counter, because a patient who visited two years ago did not.
...
Linking is authorised by a [link token](/docs/pr-41/docs/hiecm/v3/getting-started/glossary#link-token), not by your session token alone. The token ties your facility to one patient's ABHA address.
...
The full list is on [M2 errors](/docs/pr-41/docs/hiecm/v3/api/m2/errors).

## Where this is implemented

- [Hospital, lab and pharmacy systems](/docs/pr-41/docs/hiecm/v3/concepts/hip-hiu), what a facility builds to do the linking.
- [M2 Health Information Provider, Health Information Provider Services](/docs/pr-41/docs/hiecm/v3/api/m2), the call order for all three routes.
- [Consent](/docs/pr-41/docs/hiecm/v3/concepts/consent), what happens once somebody asks for a linked care context.
- [How a record travels](/docs/pr-41/docs/hiecm/v3/concepts/data-flow), what you do when that request arrives.
- [PHR applications](/docs/pr-41/docs/hiecm/v3/concepts/participants/phr), the patient side of discovery and linking.
