...
A facility publishing a record through your software is the [HIP](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#hip), the health information provider in [ABDM](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#abdm). Your job has two halves. Identify the patient by their [ABHA](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#abha) address at registration, which is [M1](/docs/pr-15/docs/hiecm/v3/api/m1). Make the records the facility creates discoverable and shareable, which is [M2](/docs/pr-15/docs/hiecm/v3/api/m2).

## Before you start

- **A valid facility ID, registered in the HIP role.** That authorises the facility to create health records and share them with whoever asks to read them as the [HIU](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#hiu). Registration lives in the [Health Facility Registry](/docs/pr-15/docs/hiecm/v3/registries).
- **A link to an [HRP](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#hrp), a health repository provider**, so discovery reaches you. The facility is the HRP in most integrations, and the [HMIS](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#hmis) or [LMIS](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#lmis) you run is how it holds the records. Where a facility's records sit with another organisation, that organisation is the HRP.
...
1. **Health records in the right format.** A [FHIR](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#fhir) R4 bundle following the NRCES profiles at <https://nrces.in/ndhm/fhir/r4/index.html>, either simple with a PDF or image attachment or structured with coded information. There are eight record types, all mandatory for an HMIS. See [FHIR and health record formats](/docs/pr-15/docs/hiecm/v3/concepts/fhir).
2. **Care contexts.** The unit that attaches to an ABHA address. The [HIE-CM](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#hie-cm) is data blind and holds two fields per care context: your internal reference ID, and a display name with nothing clinical in it. Use one per outpatient visit and one per inpatient admission.
3. **Linking.** Two routes: HIP initiated with their ABHA address, and discovery when the patient comes looking from their [PHR](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#phr) app. Linking needs a linking token, stored at registration, valid 6 months, regenerated with the generate link token call. See [Care contexts and linking](/docs/pr-15/docs/hiecm/v3/concepts/linking).
...
- **Fetching records as the HIU**, unless the facility also reads records from elsewhere. That is [M3](/docs/pr-15/docs/hiecm/v3/api/m3), a separate integration.
- **[UHI](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#uhi).** Appointments, ambulances and pharmacy ordering run on a different gateway.
- **[NHCX](/docs/pr-15/docs/hiecm/v3/getting-started/glossary#nhcx).** Claims exchange runs on a different gateway again, and no endpoint on it has been documented here yet.

## What to read next

- [M1 overview](/docs/pr-15/docs/hiecm/v3/api/m1) and the [M1 API reference](/docs/pr-15/reference/hiecm-m1).
- [M2 overview](/docs/pr-15/docs/hiecm/v3/api/m2) and the [M2 API reference](/docs/pr-15/reference/hiecm-m2).
- [M2 errors](/docs/pr-15/docs/hiecm/v3/api/m2/errors), the full ABDM error code list.
- [M2 steps and calls](/docs/pr-15/reference/hiecm-m2), what sandbox exit asks you to demonstrate.
- [PHR applications](/docs/pr-15/docs/hiecm/v3/concepts/phr), the other side of every flow here.
