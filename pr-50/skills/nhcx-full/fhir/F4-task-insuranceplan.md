...
- Both inputs are required, one of each type: the policy number and the facility HFR ID. Without both the send is refused: "A plan request needs both the case's policy code and the facility's HFR ID."
...
| `input[]` (policy) | `type.coding[0]` system `https://nrces.in/ndhm/fhir/r4/CodeSystem/ndhm-task-input-type-code`, code `policyNumber`; `valueString` D9 `claim.policy_code` | 1..1, required; also stored in D10 `claim_plan.policy_code` |
| `input[]` (provider) | same system, code `providerId`; `valueString` D1 `organization.identifier_value` (the facility HFR ID) | 1..1, required; also stored in D10 `claim_plan.provider_id` |
