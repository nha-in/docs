...
`https://nha-in.github.io/docs/pr-48/skills/nhcx-index.json` lists every NHCX skill, its archive and the exact files it is made of. A skill is 66 to 131 files across its folders, so take the archive rather than fetching files one at a time.
...
- The skills are snapshots. The current documentation lives at https://nha-in.github.io/docs/pr-48/docs/nhcx/v1; prefer it, and the MCP server when connected, over any downloaded copy that has aged.
