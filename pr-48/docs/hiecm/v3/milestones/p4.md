...
A health locker is a [PHR](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#phr) application that keeps a person's health records for the long term, under their control. It hears about new records through a subscription on their [ABHA address](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#abha-address), fetches them with consent, and stores them.
...
- Processing documents the person uploads needs certification as a Health Locker ([Health Repository Provider](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#hrp)), which requires [M2](/docs/pr-48/docs/hiecm/v3/milestones/m2).

## Who needs it

A PHR application that stores a person's records rather than only displaying them. A health locker builds on [P3 Subscription](/docs/pr-48/docs/hiecm/v3/milestones/p3): it uses the same subscription and consent calls, and adds the locker setup and settings calls on this page.
...
A notification carries no health records. The records arrive only through the health information request that follows a granted consent, as described in [P3 Subscription](/docs/pr-48/docs/hiecm/v3/milestones/p3).
...
Every call carries `REQUEST-ID`, `TIMESTAMP`, `X-CM-ID` and the gateway session token. The calls about a patient also carry `X-AUTH-TOKEN`, the person's own login token from [P1 Registration and login](/docs/pr-48/docs/hiecm/v3/milestones/p1), which is a different token from the gateway session token.
...
- The calls and base URLs: [P4 API reference](/docs/pr-48/reference/hiecm-p4).
- Subscriptions and consent from the patient's side: [P3 Subscription](/docs/pr-48/docs/hiecm/v3/milestones/p3).
- Where a locker fits among PHR applications: [Personal Health Record (PHR) Application](/docs/pr-48/docs/hiecm/v3/concepts/participants/phr).
