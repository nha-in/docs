...
In [ABDM](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#abdm) the request goes through the [gateway](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#gateway) and the [HIE-CM](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#hie-cm), because that is where consent is checked and routing lives. The record itself goes straight from the system that holds it to a URL the requester nominated, encrypted so only the requester can open it.

## The two sides

| Role                                                          | Who takes it                               | What it does here                                                                                                                                                        | Milestone                              |
| ------------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| [HIU](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#hiu) | The organisation or citizen asking         | Holds a granted [consent artefact](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#consent-artefact), asks for the records it covers, receives them and decrypts them | [M3](/docs/pr-48/docs/hiecm/v3/api/m3) |
| [HIP](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#hip) | The citizen or facility holding the record | Holds the records, validates the consent, packages, encrypts, signs and pushes                                                                                           | [M2](/docs/pr-48/docs/hiecm/v3/api/m2) |
...
Only then does it package the records as [FHIR](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#fhir) bundles, encrypt, sign with its long term private key, and send with the transaction id to the data push URL.
...
The scheme is [Elliptic Curve Diffie-Hellman](/docs/pr-48/docs/hiecm/v3/getting-started/glossary#ecdh) key exchange on Curve25519, with AES-GCM for the payload and HKDF to derive the session key. Only the HIU holding valid consent can read the data, and the design gives perfect forward secrecy: key material compromised later does not expose data exchanged earlier.
...
- [Hospital, lab and pharmacy systems](/docs/pr-48/docs/hiecm/v3/concepts/hip-hiu), which side the facility you act for is on.
- [M2 Health Information Provider: Create and link records](/docs/pr-48/docs/hiecm/v3/api/m2), the sending side.
- [M3 Health Information User: Fetch data with consent](/docs/pr-48/docs/hiecm/v3/api/m3), the receiving side.
- [Consent](/docs/pr-48/docs/hiecm/v3/concepts/consent), the artefact this flow depends on.
- [FHIR and health record formats](/docs/pr-48/docs/hiecm/v3/concepts/fhir), what is inside the payload.
