...
The most common failure is sealing with the wrong public key: your own, or a stale one. The exchange accepts the message, because the envelope is fine, and the recipient cannot open it. [The recipient cannot decrypt](/docs/pr-47/docs/nhcx/v1/troubleshooting/the-recipient-cannot-decrypt) covers that case.

## Where a message stands

Every envelope carries a status word. The sender sets it to say whether this is a new request, an interim answer, a final answer or a refusal. The exchange tracks its own states too: queued, delivered, or stopped after delivery failed. The values and the rules for each are in [The JWE message format](/docs/pr-47/docs/nhcx/v1/getting-started/jwe-message-format).
...
Each refusal carries a code. [Error codes](/docs/pr-47/docs/nhcx/v1/reference/error-codes) lists every code with its message and what to do. [Reading error codes](/docs/pr-47/docs/nhcx/v1/reference/error-code-guide) explains the code families.
...
- [The JWE message format](/docs/pr-47/docs/nhcx/v1/getting-started/jwe-message-format): every field, status value, receipt and error code.
- [Building and sending a JWE](/docs/pr-47/docs/nhcx/v1/getting-started/building-and-sending-a-jwe): seal and post your first message.
- [Envelope fields](/docs/pr-47/docs/nhcx/v1/reference/envelope-fields): each header checked against its sources.
